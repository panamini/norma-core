# PR258 RLS Boundary and Auth0 Sandbox Qualification Contract
...
Auth0 sandbox qualification. It does not select a production authorization
...
- Scalekit is the greenfield challenger and is not implemented here.
...
6. Service-role, superuser, and bypass-RLS credentials are forbidden for
   user-scoped requests. Migrations and administrative operations are separate
   non-user paths.
...
confirmation authorizes an isolated sandbox, qualification must prove all of
the following with disposable test identities and no production data:

1. Auth0 issues an RS256/OIDC-discoverable access token with a `kid`, the exact
   MCP audience, a non-empty subject, and `norma:structured-analyze`.
2. The Railway verifier accepts the valid token and rejects wrong issuer,
   audience, signature, expiry, subject, resource, and scope.
3. The provider-neutral adapter produces the same context shape without
   exposing Auth0-specific fields to the database layer.
4. Two tenants cannot read or mutate one another's rows, including through a
   reused pooled connection.
5. Missing or ambiguous tenant context, expired context, wrong scope, and
   transaction-context leakage all fail closed.
6. No user-scoped path uses a Supabase service/secret key or another
   bypass-RLS credential.
7. The qualification records only bounded pass/fail evidence and retains no
   access token, raw claims, user email, prompt, row data, or private URL.

The sandbox must stop with `NEEDS_DECISION` if the exact Auth0 token cannot
carry the required authorization context, if the RLS context cannot be bound
to one transaction, or if any cross-tenant or bypass-RLS path is observed.

Scalekit and WorkOS are comparison/qualification work only. No provider
adapter, resource, configuration, or token flow for either is part of PR258.
...
- The current MCP contract is Auth0 access-token based and requires the
