...
The pre-centroid triangle-derived rail is current through PR240, merged at
...
without changing the construction geometry. The current changeset adds exactly
one opt-in centroid as the arithmetic mean of the admitted triangle's three
canonical vertices. Each family is independently controlled, off by default,
retains stable triangle and side or vertex provenance, and revalidates its
parents fail-closed. No layer exposes a circumcenter, incenter, orthocenter,
ratio, harmonic claim, or Core authority. Selecting a layer cannot adopt
geometry, confirm the selection, or run Core.
...
7. Each enabled triangle-derived family requires the enabled explicit triangle. The line families derive exactly three constructions: vertex-to-opposite-midpoint medians, perpendicular bisectors of opposite-side support lines, internal angle bisectors, or vertex-to-opposite-support-line altitudes. **Centroïde** derives exactly one point as the arithmetic mean of the three canonical triangle vertices. Disabling or invalidating the triangle removes all dependent layers. No circumcenter, incenter, or orthocenter is exposed.
...
11. After confirmation, the construction analysis reports deterministic image-plane directions, frame-edge contacts, support-line/format-diagonal intersections, normalized positions, smaller/supplementary projected angles at bounded line junctions, metrics for explicitly parented triangles, exactly three constructions for each enabled triangle line family, and one `triangleCentroids` point when **Centroïde** is enabled. Every construction remains `candidateEvidenceOnly=true`, `sourceTruth=false`, and `coreAuthority=false`. It never infers another triangle center, artistic intent, physical geometry, a vanishing point, or harmonic meaning.
...
| Triangle medians | Implemented and merged in PR234 | Exactly three canonical vertex-to-opposite-midpoint segments for one admitted triangle; stable parent provenance and identity; off by default; no automatic center inference or Core authority |
...
| Triangle centroid | Implemented in the bounded centroid changeset | Exactly one arithmetic-mean point for one admitted canonical triangle; deterministic parent identity; off by default; `candidateEvidenceOnly=true`, `sourceTruth=false`, and `coreAuthority=false` |
| Other triangle centers | Deferred | Require separate bounded contracts and evidence; no automatic enumeration, harmonic classification, or inference from line intersections |
