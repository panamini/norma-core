...
function directSegmentPairMode(){return state.guidedAnalysisGoal==="compare-two-lengths"&&!declaredSpatialMeasurementMode()}
...
function appendManualSegmentCard(item){const card=document.createElement("div"),copy=document.createElement("div"),kindNode=document.createElement("span"),title=document.createElement("strong"),reason=document.createElement("span"),input=document.createElement("input"),pixelEvidence=document.createElement("div"),remove=document.createElement("button"),pairIndex=state.manualSegmentCandidateIds.indexOf(item.id);card.className="candidate manual-candidate";card.setAttribute("data-primitive-kind","segment");card.setAttribute("data-candidate-id",item.id);card.setAttribute("data-manual-segment-candidate-id",item.id);kindNode.className="candidate-kind";kindNode.textContent="Mesure "+(pairIndex===0?"A":"B")+" · segment manuel";title.textContent=String(state.reviewedCandidates.indexOf(item)+1)+" · "+item.label;reason.textContent=item.reason;copy.append(kindNode,title,reason);input.type="checkbox";input.checked=state.selectedGuides.has(item.id);input.disabled=state.completed||state.confirming;input.setAttribute("aria-label","Confirmer comme guide visuel : "+item.label);input.addEventListener("change",()=>{if(input.checked)state.selectedGuides.add(item.id);else state.selectedGuides.delete(item.id);updateConstructionControls();syncOverlaySelection();updateMeasurementRatioControls();persistSelection();updateConfirm()});pixelEvidence.className="pixel-evidence";pixelEvidence.setAttribute("data-pixel-candidate-id",item.id);remove.type="button";remove.className="manual-candidate-remove";remove.textContent="Supprimer";remove.disabled=state.completed||state.confirming||state.pixelRefinementRunning;remove.setAttribute("aria-label","Supprimer la mesure "+(pairIndex===0?"A":"B"));remove.addEventListener("click",()=>removeManualSegment(item.id));card.append(input,copy,pixelEvidence,remove);candidateList.append(card)}
...
candidateList.replaceChildren();overlay.classList.remove("locked");state.proposalCandidates=prepared.candidates.map(item=>JSON.parse(JSON.stringify(item)));state.manualSegmentCandidateIds=state.reviewedCandidates.filter(isManualSegmentCandidate).map(item=>item.id).slice(0,2);state.manualSegmentCandidateId=state.manualSegmentCandidateIds[0]??null;restorePixelRefinementState(activePrepared);
