...
function restoredManualSegmentFor(prepared){const saved=publicWidgetState().manualSegmentState;if(!saved||typeof saved!=="object"||Object.keys(saved).sort().join("|")!=="fileId|geometry"||saved.fileId!==state.activePayload?.fileId||prepared.candidates.length>=MAX_REVIEW_CANDIDATES||!saved.geometry||prepared.candidates.some(item=>item.id===saved.geometry.id))return null;const geometry=saved.geometry,candidate={id:geometry.id,label:"Segment ajouté manuellement",role:"secondary-subject",reason:"Guide tracé explicitement par l’utilisateur dans le widget; preuve candidate à vérifier avant confirmation.",x:geometry.x,y:geometry.y,width:geometry.width,height:geometry.height,primitive:clonePrimitive(geometry.primitive)};return isManualSegmentCandidate(candidate)?candidate:null}
...
function completedPreparedFor(payload,saved,completed){const prepared=payload?.prepared;if(!prepared||!Array.isArray(prepared.candidates)||!completed)return null;if(completed.candidateSetIdentity===prepared.candidateSetIdentity)return prepared;if(!isStoredIdentity(completed.candidateSetIdentity)||saved.reviewedProposalCandidateSetIdentity!==completed.candidateSetIdentity||!Array.isArray(saved.reviewedCandidateGeometry)||!Array.isArray(completed.reviewedCandidateGeometry)||saved.reviewedCandidateGeometry.length!==completed.reviewedCandidateGeometry.length)return null;if(saved.reviewedCandidateGeometry.length===prepared.candidates.length)return{...prepared,candidateSetIdentity:completed.candidateSetIdentity};if(saved.reviewedCandidateGeometry.length!==prepared.candidates.length+1||prepared.candidates.length>=MAX_REVIEW_CANDIDATES)return null;const manualState=saved.manualSegmentState,geometry=manualState?.geometry;if(!manualState||typeof manualState!=="object"||Object.keys(manualState).sort().join("|")!=="fileId|geometry"||manualState.fileId!==payload.fileId||!geometry||JSON.stringify(geometry)!==JSON.stringify(saved.reviewedCandidateGeometry.at(-1))||prepared.candidates.some(item=>item.id===geometry.id))return null;const manual={id:geometry.id,label:"Segment ajouté manuellement",role:"secondary-subject",reason:"Guide tracé explicitement par l’utilisateur dans le widget; preuve candidate à vérifier avant confirmation.",x:geometry.x,y:geometry.y,width:geometry.width,height:geometry.height,primitive:clonePrimitive(geometry.primitive)};return isManualSegmentCandidate(manual)?{...prepared,candidateSetIdentity:completed.candidateSetIdentity,candidates:[...prepared.candidates,manual]}:null}
