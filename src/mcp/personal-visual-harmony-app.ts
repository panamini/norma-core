...
const PERCEPTION_MAX_STATUS_POLLS=160,PERCEPTION_STATUS_POLL_DELAY_MS=2000,PERCEPTION_CLIENT_WORKFLOW_TIMEOUT_MS=320000,PERCEPTION_TOOL_CALL_TIMEOUT_MS=15000;
...
async function pollPerceptionJob(payload,jobId,expiresAt,expectedPayloadIdentity){const expiresAtMs=Date.parse(expiresAt);if(!Number.isFinite(expiresAtMs))throw new Error("invalid perception job expiry");const pollDeadlineMs=Math.min(expiresAtMs,Date.now()+PERCEPTION_CLIENT_WORKFLOW_TIMEOUT_MS),statusArgs={sessionId:payload.sessionId,candidateSetIdentity:payload.prepared.candidateSetIdentity,appCapability:payload.perceptionAppCapability,jobId};let remainingPolls=PERCEPTION_MAX_STATUS_POLLS,finalPollPending=false;while(Date.now()<pollDeadlineMs&&(remainingPolls>0||finalPollPending)){if(state.activePayloadIdentity!==expectedPayloadIdentity||state.completed)return;const finalPoll=finalPollPending;finalPollPending=false;if(!finalPoll)remainingPolls-=1;const statusTimeoutMs=finalPoll?Math.min(PERCEPTION_TOOL_CALL_TIMEOUT_MS,Math.max(1,pollDeadlineMs-Date.now())):PERCEPTION_TOOL_CALL_TIMEOUT_MS;const response=await callAppTool(PERCEPTION_STATUS_TOOL,statusArgs,statusTimeoutMs);if(await applyPerceptionStatusResponse(payload,response))return;if(finalPoll)break;if(remainingPolls===0){const remainingMs=Math.max(0,pollDeadlineMs-Date.now());if(remainingMs<=0)break;finalPollPending=true;await new Promise(resolve=>setTimeout(resolve,Math.max(1,Math.floor(remainingMs/2))));continue}const remainingMs=Math.max(0,pollDeadlineMs-Date.now());if(remainingMs>0)await new Promise(resolve=>setTimeout(resolve,Math.min(PERCEPTION_STATUS_POLL_DELAY_MS,remainingMs)))}throw perceptionClientError("perception_poll_timeout","perception status polling timed out")}
