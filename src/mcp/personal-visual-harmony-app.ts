...
function declaredSpatialSourceIdentity(){const prepared=state.payload?.prepared,binding=spatialWorkflowBinding();if(binding.status!=="legacy"&&binding.status!=="bound"&&binding.status!=="manual")return null;if((prepared?.contractVersion===2||prepared?.contractVersion===3)&&typeof prepared.sourceImageContentIdentity==="string")return prepared.sourceImageContentIdentity;return typeof prepared?.sourceImageReferenceIdentity==="string"?prepared.sourceImageReferenceIdentity:null}
...
function updateSpatialRecoveryUi(){if(!spatialRecovery)return;const manual=spatialWorkflowBinding().status==="manual",required=spatialRecoveryRequired(),busy=state.spatialRecoveryRunning||state.confirming||state.perceptionRunning||state.completed,selectedRectangleCount=manualSelectedRectangleIds().length,needsSelectedPair=required&&state.payload?.prepared?.contractVersion===1&&(state.payload?.prepared?.candidates?.length??0)>10;spatialRecovery.hidden=!required&&!manual;spatialRecoveryActions.hidden=manual;spatialRecoveryStatus.textContent=manual?"MODE MANUEL · preuves non liées · aucune liaison SAM A/B. Les deux rectangles restent des candidats explicitement sélectionnés.":needsSelectedPair&&selectedRectangleCount!==2?"Sélectionnez exactement deux rectangles pour créer une nouvelle session SAM A/B bornée, sans lancer le provider ni Core.":"La revue SAM A/B est interrompue. Choisissez explicitement une nouvelle session SAM ou un repli manuel non lié.";restartSpatialReview.disabled=!required||busy||needsSelectedPair&&selectedRectangleCount!==2;continueManualSpatial.disabled=!required||busy||selectedRectangleCount!==2;continueManualSpatial.title=manualSelectedRectangleIds().length===2?"":"Sélectionnez exactement deux rectangles avant le repli manuel."}
...
async function prepareSpatialRecoveryPayload(payload,candidateSnapshot){const expectedPayloadIdentity=state.activePayloadIdentity,fileApi=window.openai?.getFileDownloadUrl;if(typeof fileApi!=="function")throw new Error("file API unavailable");const freshDownload=await withPerceptionDeadline(()=>fileApi({fileId:payload.fileId}),PERCEPTION_TOOL_CALL_TIMEOUT_MS,"perception_file_timeout"),downloadUrl=freshDownload?.downloadUrl;if(typeof downloadUrl!=="string"||!downloadUrl.startsWith("https://"))throw new Error("invalid fresh image URL");const image={download_url:downloadUrl,file_id:payload.fileId};if(typeof payload.sourceImageMediaType==="string"&&payload.sourceImageMediaType.length>0)image.mime_type=payload.sourceImageMediaType;const response=await callAppTool(PREPARE_TOOL,{image,candidates:candidateSnapshot});if(state.activePayloadIdentity!==expectedPayloadIdentity)throw new Error("stale spatial recovery preparation");const fresh=findPayload(response);if(!fresh||fresh.stage!=="confirmation_required"||fresh.fileId!==payload.fileId||fresh.prepared?.contractVersion!==1||!samePreparedReviewCandidates(candidateSnapshot,fresh.prepared?.candidates))throw new Error("spatial recovery preparation mismatch");if(fresh.perceptionRecoveryAvailable!==true||typeof fresh.perceptionAppCapability!=="string"||fresh.perceptionAppCapability.length<32||!Array.isArray(fresh.perceptionModes)||!fresh.perceptionModes.includes("two-object-spatial"))throw new Error("spatial recovery SAM capability missing");return fresh}
