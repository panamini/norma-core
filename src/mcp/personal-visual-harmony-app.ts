...
async function hydrate(payload=currentPayload(),structured=window.openai?.toolOutput,{forceImageReload=false}={}){if(!payload)return;const identity=payloadIdentity(payload),imageChanged=state.imageLoadFileId!==payload.fileId||state.imageLoadPayloadIdentity!==identity;if(state.activePayloadIdentity!==null&&state.activePayloadIdentity!==identity)resetManualSegmentGesture();state.pendingStructuredContent=structured;state.activePayload=payload;state.activePayloadIdentity=identity;if(payload.stage==="confirmation_required"&&state.confirming){state.payload=payload;return}if(payload.stage==="confirmation_required"||!state.payload||state.payload.fileId!==payload.fileId)state.payload=payload;if(imageChanged){state.imageReady=false;state.dimensions=null}if(payload.overlaySvg)overlay.innerHTML=safeSvg(payload.overlaySvg);if(payload.stage==="confirmation_required"){renderCandidates(payload.prepared);const imageLoaded=await loadImage(payload.fileId,identity,{force:forceImageReload});if(!imageLoaded||state.activePayloadIdentity!==identity)return;if(state.pixelRefinementEnabled)await refreshPixelRefinements(state.payload,identity);if(state.activePayloadIdentity!==identity)return;const revalidationPayload=state.payload,completed=completedWidgetStateFor(revalidationPayload);if(completed&&!state.confirming&&!state.completed)await revalidateCompleted(revalidationPayload,completed,identity);return}if(payload.stage==="completed"){if(payload.fileId&&!await loadImage(payload.fileId,identity,{force:forceImageReload}))return;if(state.activePayloadIdentity!==identity)return;state.displayedPayload=payload;restoreGuidedAnalysisGoal();renderGuidedAnalysisGoals();renderResult(payload,structured)}}
