...
function applyGuidedAnalysisGoal(id){if(state.confirming||state.spatialRecoveryRunning||twoObjectSpatialWorkflowActive()&&id!=="compare-two-lengths")return;const prepared=state.payload?.prepared,perceptionDirectBlocked=prepared?.workflowMode!=="two-object-spatial"&&Boolean(prepared?.perceptionReceiptIdentity),candidateCount=Array.isArray(state.reviewedCandidates)?state.reviewedCandidates.length:Array.isArray(prepared?.candidates)?prepared.candidates.length:0,directPairCandidates=Array.isArray(state.reviewedCandidates)&&state.reviewedCandidates.length>0?state.reviewedCandidates:Array.isArray(prepared?.candidates)?prepared.candidates:[],availableRectangleCount=directPairCandidates.filter(item=>!item.primitive||item.primitive.kind==="rectangle").length,directPairCapacityBlocked=id==="compare-two-lengths"&&prepared?.workflowMode!=="two-object-spatial"&&candidateCount>MAX_REVIEW_CANDIDATES-2&&(state.payload?.perceptionRecoveryAvailable!==true||availableRectangleCount<2);if(id==="compare-two-lengths"&&perceptionDirectBlocked){state.manualSegmentMode=false;state.manualSegmentAnchor=null;guidedGoalStatus.textContent="La mesure directe A/B sur une préparation assistée appartient à une phase séparée. Revenez à une préparation manuelle pour tracer A et B; SAM reste disponible dans son parcours dédié.";return}if(directPairCapacityBlocked){state.manualSegmentMode=false;state.manualSegmentAnchor=null;guidedGoalStatus.textContent=state.payload?.perceptionRecoveryAvailable===true?"La récupération A/B exige deux rectangles disponibles. Repréparez l’image avec au plus dix candidats ou choisissez une image contenant deux rectangles avant de tracer A et B.":"La mesure directe A/B exige deux emplacements libres. Repréparez l’image avec au plus dix candidats avant de tracer A et B.";return}const goal=GUIDED_ANALYSIS_GOALS.find(value=>value.id===id)||GUIDED_ANALYSIS_GOALS[0],changed=state.guidedAnalysisGoal!==goal.id;state.guidedAnalysisGoal=goal.id;state.visibleKinds=new Set(visibleKindsForGuidedAnalysisGoal(goal));if(changed){state.measurementRatioEnabled=false;state.measurementRatioRefs=[];state.declaredSpatialMeasurementPlanRevision+=1;state.declaredSpatialMeasurementPlanInputKey=null;state.declaredSpatialMeasurementPlan=null;state.declaredSpatialMeasurementPlanBuilding=false}if(goal.id!=="compare-two-lengths"&&(state.manualSegmentMode===true||state.manualSegmentAnchor!==undefined&&state.manualSegmentAnchor!==null)){resetManualSegmentGesture();updateManualSegmentControls()}updateGuidedAnalysisGoalButtons();updateFamilyFilterButtons();syncFamilyVisibility();if(typeof updateMeasurementRatioControls==="function")updateMeasurementRatioControls();if(typeof updatePerceptionUi==="function")updatePerceptionUi();guidedGoalStatus.textContent=goal.effect;persistGuidedAnalysisGoal();if(changed&&goal.id==="compare-two-lengths"){startFreshSpatialSessionIfNeeded();if(candidateCount<=MAX_REVIEW_CANDIDATES-2&&state.payload?.prepared?.workflowMode!=="two-object-spatial"&&!state.spatialRecoveryRunning){state.manualSegmentMode=true;state.manualSegmentAnchor=null;if(typeof updateManualSegmentControls==="function")updateManualSegmentControls()}}}
...
function findMeasurementPairConfirmation(value,depth=0){if(depth>7||value===null||typeof value!=="object")return null;const confirmation=value.measurementPairConfirmation;if(confirmation?.contractId==="norma.personal-visual-harmony-measurement-pair-confirmation@1"&&confirmation.status==="completed"&&confirmation.explicitConfirmation===true&&confirmation.measurementOnly===true&&confirmation.providerCalls===0&&confirmation.coreAuthority===false&&confirmation.coreRun===false&&confirmation.coreExecutionCount===0&&Array.isArray(confirmation.confirmedVisualGuideCandidateIds)&&confirmation.confirmedVisualGuideCandidateIds.length===2&&isStoredIdentity(confirmation.confirmationIdentity))return confirmation;for(const entry of Object.values(value)){const found=findMeasurementPairConfirmation(entry,depth+1);if(found)return found}return null}
