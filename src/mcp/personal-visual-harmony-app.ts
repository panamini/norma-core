...
export const PERSONAL_VISUAL_HARMONY_WIDGET_URI =
  "ui://widget/norma-personal-visual-harmony-v17.html";
...
async function perceptionDownloadUrl(payload){if(validPerceptionDownloadUrl(state.downloadUrl))return state.downloadUrl;if(validPerceptionDownloadUrl(payload?.sourceImageDownloadUrl))return payload.sourceImageDownloadUrl;const fileApi=window.openai?.getFileDownloadUrl;if(typeof fileApi!=="function")throw new Error("file API unavailable");const freshDownload=await withPerceptionDeadline(()=>fileApi({fileId:perceptionSourceFileId(payload)}),PERCEPTION_TOOL_CALL_TIMEOUT_MS,"perception_file_timeout"),downloadUrl=freshDownload?.downloadUrl;if(!validPerceptionDownloadUrl(downloadUrl))throw new Error("invalid fresh image URL");return downloadUrl}
