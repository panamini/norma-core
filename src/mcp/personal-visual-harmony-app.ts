...
function multiPerceptionStartBlocked(payload=state.payload){return state.manualSegmentCandidateId!==null&&perceptionWorkflowArgs(payload).workflowMode==="two-object-spatial"}
...
function updateConfirm(){const spatial=declaredSpatialMeasurementMode(),spatialBinding=spatial?spatialWorkflowBinding():null,noCoreRectangle=coreSelectedIds().length===0,incompleteSpatialPlan=spatial&&(!state.measurementRatioEnabled||state.declaredSpatialMeasurementPlanBuilding||state.declaredSpatialMeasurementPlan===null),incompleteMeasurementRatio=!spatial&&state.measurementRatioEnabled&&measurementRatioRequest()===undefined,multiLocked=multiPerceptionReviewLocked();confirmButton.disabled=state.completed||state.confirming||state.pixelRefinementRunning||state.perceptionRunning||multiLocked||!state.imageReady||noCoreRectangle||incompleteSpatialPlan||incompleteMeasurementRatio||!state.payload;if(!state.completed&&!state.confirming)confirmButton.textContent=spatial?"Calculer le rapport A/B":"Analyser les rapports sélectionnés";updateManualSegmentControls();if(!state.completed&&!state.confirming&&state.imageReady&&multiLocked)statusNode.textContent="Terminez l’observation de l’objet B avant toute édition ou confirmation.";else if(!state.completed&&!state.confirming&&state.imageReady&&spatialBinding?.message)statusNode.textContent=spatialBinding.message;else if(!state.completed&&!state.confirming&&state.imageReady&&noCoreRectangle)statusNode.textContent=spatial?"Sélectionnez exactement deux zones A et B sur l’image ou dans la liste.":"Vérifiez puis sélectionnez au moins une zone à analyser.";else if(!state.completed&&!state.confirming&&state.imageReady&&incompleteSpatialPlan)statusNode.textContent="Choisissez deux zones, puis les mesures A et B. Vérifiez leurs traits sur l’image avant de confirmer.";else if(!state.completed&&!state.confirming&&state.imageReady&&incompleteMeasurementRatio)statusNode.textContent="Choisissez deux mesures distinctes, ou désactivez la comparaison."}
