...
function adjustedEllipseCandidate(item,handle,dx,dy){const primitive=item?.primitive;if(primitive?.kind!=="ellipse")return item;const axes=ellipseAxes(primitive);let nextPrimitive=primitive;if(handle==="radius-x"){const delta=dx*axes.x.x+dy*axes.x.y;nextPrimitive={...primitive,radiusX:rounded(Math.max(.005,Math.min(1,primitive.radiusX+delta)))}}else if(handle==="radius-y"){const delta=dx*axes.y.x+dy*axes.y.y;nextPrimitive={...primitive,radiusY:rounded(Math.max(.005,Math.min(1,primitive.radiusY+delta)))}}else{nextPrimitive={...primitive,center:{x:rounded(Math.max(0,Math.min(1,primitive.center.x+dx))),y:rounded(Math.max(0,Math.min(1,primitive.center.y+dy)))}}}return candidateWithPrimitive(item,nextPrimitive)}
