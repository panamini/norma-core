...
function spatialRecoveryRequired(){if(state.guidedAnalysisGoal!=="compare-two-lengths"||state.completed)return false;const binding=spatialWorkflowBinding(),prepared=state.payload?.prepared,modes=Array.isArray(state.payload?.perceptionModes)?state.payload.perceptionModes:[],unboundUnavailable=binding.status==="unbound"&&perceptionToggle.hidden,oversizedLegacyUnavailable=binding.status==="legacy"&&prepared?.contractVersion===1&&(prepared?.candidates?.length??0)>10&&modes.includes("legacy")&&!modes.includes("two-object-spatial")&&perceptionToggle.hidden;return state.multiPerceptionTerminalState!==null||state.perceptionReconciliationBlocked||binding.status==="invalid"||unboundUnavailable||oversizedLegacyUnavailable}
