...
automaticPreview.hidden=manual||!preview;automaticPreview.dataset.stale=String(state.automaticPreviewStale);if(automaticPreview.hidden)return;
...
state.displayedPayload=payload;state.completed=true;state.completedReviewEditable=true;state.editableOverlayMarkup=overlay.innerHTML;overlay.classList.remove("locked");candidateList.querySelectorAll("input").forEach(input=>input.disabled=false);confirmButton.style.display="none";
