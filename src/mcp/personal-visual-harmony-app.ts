...
export const PERSONAL_VISUAL_HARMONY_WIDGET_URI =
  "ui://widget/norma-personal-visual-harmony-v19.html";
...
function perceptionSourceFileId(payload){const sourceFileId=typeof payload?.fileId==="string"&&payload.fileId.length>0?payload.fileId:state.imageLoadFileId;if(typeof sourceFileId!=="string"||sourceFileId.length===0)throw new Error("perception source file identity unavailable");return sourceFileId}
async function perceptionDownloadUrl(payload){const fileApi=window.openai?.getFileDownloadUrl;if(typeof fileApi==="function"){const freshDownload=await withPerceptionDeadline(()=>fileApi({fileId:perceptionSourceFileId(payload)}),PERCEPTION_TOOL_CALL_TIMEOUT_MS,"perception_file_timeout"),downloadUrl=freshDownload?.downloadUrl;if(!validPerceptionDownloadUrl(downloadUrl))throw new Error("invalid fresh image URL");return downloadUrl}if(validPerceptionDownloadUrl(state.downloadUrl))return state.downloadUrl;if(validPerceptionDownloadUrl(payload?.sourceImageDownloadUrl))return payload.sourceImageDownloadUrl;throw new Error("file API unavailable")}
...
async function callAppTool(name,args,timeoutMs=0){const startedAt=Date.now();if(typeof window.openai?.callTool==="function")return withPerceptionDeadline(()=>window.openai.callTool(name,args),timeoutMs,"perception_tool_timeout");await withPerceptionDeadline(()=>bridgeReady,timeoutMs,"perception_tool_timeout");const remainingMs=timeoutMs>0?Math.max(1,timeoutMs-(Date.now()-startedAt)):0;try{return await rpcRequest("tools/call",{name,arguments:args},remainingMs)}catch(error){document.documentElement.setAttribute("data-norma-last-error","tools-call");throw error}}
