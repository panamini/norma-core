...
const modes=state.payload?.perceptionModes, candidates=state.reviewedCandidates.length>0?state.reviewedCandidates:prepared?.candidates||[],hasPrimary=candidates.some(item=>primitiveKind(item)==="rectangle"&&item.role==="primary-subject"),hasSecondary=candidates.some(item=>primitiveKind(item)==="rectangle"&&item.role==="secondary-subject");if(Array.isArray(modes)&&modes.includes("two-object-spatial")&&hasPrimary&&hasSecondary)return{status:"unbound",candidateIds:[],message:"Les rectangles Objet A/B affichés ne sont pas liés à une revue spatiale SAM 3. Lancez « Proposer l’objet A », puis « Proposer l’objet B » ; aucune mesure ni Core ne sera lancé."};return{status:"legacy",candidateIds:[],message:null}}
function selectedSpatialRectangles(){if(!state.dimensions)return[];const binding=spatialWorkflowBinding(),selected=state.reviewedCandidates.filter(item=>primitiveKind(item)==="rectangle"&&state.selected.has(item.id));if(binding.status!=="legacy"){if(binding.status!=="bound"||selected.length!==2||binding.candidateIds.some(candidateId=>!selected.some(item=>item.id===candidateId)))return[];return binding.candidateIds.map(candidateId=>state.reviewedCandidates.find(item=>item.id===candidateId)).filter(Boolean).map(candidate=>({candidate,bounds:{x:candidate.x*state.dimensions.width,y:candidate.y*state.dimensions.height,width:candidate.width*state.dimensions.width,height:candidate.height*state.dimensions.height}}))}selected.sort((left,right)=>compareSpatialCanonical(left.id,right.id));if(selected.length!==2)return[];return selected.map(candidate=>({candidate,bounds:{x:candidate.x*state.dimensions.width,y:candidate.y*state.dimensions.height,width:candidate.width*state.dimensions.width,height:candidate.height*state.dimensions.height}}))}
...
function declaredSpatialSourceIdentity(){const prepared=state.payload?.prepared,binding=spatialWorkflowBinding();if(binding.status!=="legacy"&&binding.status!=="bound")return null;if((prepared?.contractVersion===2||prepared?.contractVersion===3)&&typeof prepared.sourceImageContentIdentity==="string")return prepared.sourceImageContentIdentity;return typeof prepared?.sourceImageReferenceIdentity==="string"?prepared.sourceImageReferenceIdentity:null}
