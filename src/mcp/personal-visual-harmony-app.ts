...
function renderFamilyFilters(prepared){familyFilters.replaceChildren();const candidates=state.reviewedCandidates.length>0?state.reviewedCandidates:prepared.candidates,kinds=[...new Set(candidates.map(primitiveKind))];for(const kind of kinds){const button=document.createElement("button");button.type="button";button.className="family-filter";button.textContent=primitiveLabel(kind);button.setAttribute("aria-pressed",String(state.visibleKinds.has(kind)));button.addEventListener("click",()=>{if(state.visibleKinds.has(kind))state.visibleKinds.delete(kind);else state.visibleKinds.add(kind);button.setAttribute("aria-pressed",String(state.visibleKinds.has(kind)));syncFamilyVisibility()});familyFilters.append(button)}}
