...
export class PersonalVisualHarmonySessionServiceV1 {
  ...
  refinePixels(input: {
    ...
    if (primitive === undefined || primitive.kind === "rectangle") {
  ...
  server.registerTool(
    ...
    ({
      ...
      return {
        content: [{
          type: "text" as const,
          text: `${structuredContent.headline}${topExplanations.length === 0 ? "" : ` ${topExplanations}`}${topQuadrilateralMeasurements.length === 0 ? "" : ` Mesures de quadrilatères dans le plan image : ${topQuadrilateralMeasurements}`}${topImagePlaneRelations.length === 0 ? "" : ` Relations séparées du plan image : ${topImagePlaneRelations}`} Les mesures et relations de guides ne sont ni des rapports harmoniques ni des mesures du monde réel.`,
        }],
        structuredContent,
        _meta: {
          normaPersonalVisualHarmony: {
...
function storedPixelRefinementStateFor(value,prepared){if(!value||typeof value!=="object"||Object.keys(value).sort().join("|")!=="adopted|candidateSetIdentity|enabled|proposals"||typeof value.enabled!=="boolean"||value.candidateSetIdentity!==prepared.candidateSetIdentity||!Array.isArray(value.proposals)||value.proposals.length>12||!Array.isArray(value.adopted)||value.adopted.length>12||!value.proposals.every(proposal=>validPixelProposal(proposal,prepared))||new Set(value.proposals.map(proposal=>proposal.candidateId)).size!==value.proposals.length)return null;const proposals=new Map(value.proposals.map(proposal=>[proposal.candidateId,proposal]));if(!value.adopted.every(entry=>entry&&Object.keys(entry).sort().join("|")==="candidateId|proposalContentIdentity"&&isStoredIdentity(entry.proposalContentIdentity)&&proposals.get(entry.candidateId)?.status==="refined"&&proposals.get(entry.candidateId)?.contentIdentity===entry.proposalContentIdentity))return null;if(!value.enabled&&(value.proposals.length!==0||value.adopted.length!==0))return null;return{enabled:value.enabled,candidateSetIdentity:value.candidateSetIdentity,proposals:[...value.proposals],adopted:[...value.adopted]}}
function restorePixelRefinementState(prepared){const stored=storedPixelRefinementStateFor(publicWidgetState().pixelRefinementState,prepared);state.pixelRefinementEnabled=stored?.enabled===true;state.pixelRefinementProposals=new Map((stored?.proposals||[]).map(proposal=>[proposal.candidateId,proposal]));state.adoptedPixelRefinements=new Map((stored?.adopted||[]).map(entry=>[entry.candidateId,entry.proposalContentIdentity]));pixelToggle.setAttribute("aria-pressed",String(state.pixelRefinementEnabled));pixelToggle.textContent=state.pixelRefinementEnabled?"Propositions pixels · activées":"Propositions pixels · désactivées"}
...
function reconcileStoredPixelAdoptions(){for(const [candidateId,proposalContentIdentity] of [...state.adoptedPixelRefinements]){const proposal=state.pixelRefinementProposals.get(candidateId),reviewed=state.reviewedCandidates.find(item=>item.id===candidateId);if(!proposal||proposal.status!=="refined"||proposal.contentIdentity!==proposalContentIdentity||!reviewed||JSON.stringify(reviewed.primitive)!==JSON.stringify(proposal.proposedGeometry))state.adoptedPixelRefinements.delete(candidateId)}}
...
function setReviewLocked(locked){const disabled=locked||state.completed;overlay.classList.toggle("locked",disabled);candidateList.querySelectorAll("input").forEach(input=>input.disabled=disabled);overlay.querySelectorAll("[data-candidate-id]").forEach(group=>{const editable=group.getAttribute("data-primitive-kind")!=="ellipse"&&!disabled;group.setAttribute("tabindex",editable?"0":"-1");group.querySelectorAll("[data-resize-handle],[data-point-handle],[data-vertex-handle]").forEach(handle=>handle.setAttribute("tabindex",editable?"0":"-1"));if(disabled)group.setAttribute("aria-disabled","true");else group.removeAttribute("aria-disabled")});updatePixelProposalUi();updateConfirm()}
...
async function callConfirmation(payload,selectedCandidateIds,confirmedVisualGuideCandidateIds,dimensions){const args={sessionId:payload.sessionId,candidateSetIdentity:payload.prepared.candidateSetIdentity,selectedCandidateIds,confirmedVisualGuideCandidateIds,sourcePixelWidth:dimensions.width,sourcePixelHeight:dimensions.height,confirmClientReviewedSelection:true,recovery:{fileId:payload.fileId,sourceImageMediaType:payload.sourceImageMediaType??null,candidates:payload.prepared.candidates}};return callAppTool(CONFIRM_TOOL,args)}
