  ...
  server.registerTool(
    ...
    ({
      ...
      const constructionSummary = topConstructions === undefined
        ? ""
        : ` Constructions dérivées optionnelles : ${String(topConstructions.supportLineExtensions.length)} prolongement(s) de droites supports, ${String(topConstructions.formatDiagonals.length)} diagonale(s) du format, ${String(topConstructions.triangles?.length ?? 0)} triangle(s) explicitement demandé(s) et ${String(topConstructions.triangleMedians?.length ?? 0)} médiane(s), séparés des observations et sans autorité Core.`;
      return {
        content: [{
          type: "text" as const,
...
<main class="shell">
  ...
  <section class="content">
    ...
    <aside class="side">
      ...
      <div class="construction-controls" aria-label="Constructions dérivées optionnelles"><button id="supportLineToggle" class="construction-toggle" type="button" aria-pressed="false">Prolongements · masqués</button><button id="formatDiagonalToggle" class="construction-toggle" type="button" aria-pressed="false">Diagonales format · masquées</button><button id="junctionAngleToggle" class="construction-toggle" type="button" aria-pressed="false" disabled>Angles jonction · masqués</button><button id="triangleToggle" class="construction-toggle" type="button" aria-pressed="false" disabled>Triangles · masqués</button><button id="triangleMedianToggle" class="construction-toggle" type="button" aria-pressed="false" disabled>Médianes · masquées</button><button id="trianglePerpendicularBisectorToggle" class="construction-toggle" type="button" aria-pressed="false" disabled>Médiatrices · masquées</button></div>
...
const CONSTRUCTION_LAYERS=["support-line-extensions","format-diagonals","junction-angles","triangles","triangle-medians","triangle-perpendicular-bisectors"];
...
function restoreConstructionGuideState(prepared){const stored=storedConstructionGuideStateFor(publicWidgetState().constructionGuideState,prepared);state.constructionLayers=new Set(stored?.layers||[]);state.visibleConstructionLayers=new Set(state.constructionLayers);if(state.constructionLayers.has("triangles")&&!triangleLayerReady(prepared,state.constructionLayers,state.selectedGuides)){state.constructionLayers.delete("triangles");state.constructionLayers.delete("triangle-medians");state.constructionLayers.delete("triangle-perpendicular-bisectors");state.visibleConstructionLayers.delete("triangles");state.visibleConstructionLayers.delete("triangle-medians");state.visibleConstructionLayers.delete("triangle-perpendicular-bisectors")}syncConstructionVisibility()}
function toggleConstructionLayer(layer){if(state.confirming||!CONSTRUCTION_LAYERS.includes(layer))return;if(state.completed){if(!state.constructionLayers.has(layer))return;if(state.visibleConstructionLayers.has(layer))state.visibleConstructionLayers.delete(layer);else state.visibleConstructionLayers.add(layer);syncConstructionVisibility();statusNode.textContent=state.visibleConstructionLayers.has(layer)?"Construction dérivée réaffichée. Le résultat confirmé et le Core restent inchangés.":"Construction dérivée masquée. Le résultat confirmé et le Core restent inchangés.";return}if((layer==="junction-angles"&&!state.constructionLayers.has("support-line-extensions"))||(layer==="triangles"&&!state.constructionLayers.has("triangles")&&!triangleLayerReady())||((layer==="triangle-medians"||layer==="triangle-perpendicular-bisectors")&&!state.constructionLayers.has(layer)&&!triangleMedianLayerReady()))return;if(state.constructionLayers.has(layer)){state.constructionLayers.delete(layer);if(layer==="support-line-extensions"){state.constructionLayers.delete("junction-angles");state.constructionLayers.delete("triangles");state.constructionLayers.delete("triangle-medians");state.constructionLayers.delete("triangle-perpendicular-bisectors")}if(layer==="triangles"){state.constructionLayers.delete("triangle-medians");state.constructionLayers.delete("triangle-perpendicular-bisectors")}}else state.constructionLayers.add(layer);if(state.constructionLayers.has("triangles")&&!triangleLayerReady()){state.constructionLayers.delete("triangles");state.constructionLayers.delete("triangle-medians");state.constructionLayers.delete("triangle-perpendicular-bisectors")}if(state.constructionLayers.has("triangle-medians")&&!triangleMedianLayerReady())state.constructionLayers.delete("triangle-medians");if(state.constructionLayers.has("triangle-perpendicular-bisectors")&&!triangleMedianLayerReady())state.constructionLayers.delete("triangle-perpendicular-bisectors");state.visibleConstructionLayers=new Set(state.constructionLayers);syncConstructionVisibility();persistReviewState();statusNode.textContent=state.constructionLayers.has(layer)?"Construction dérivée affichée. Elle reste séparée des segments observés et hors du Core.":"Construction dérivée masquée. La géométrie observée reste inchangée."}
function invalidateTriangleConstruction(){const selectedTriangleChanged=state.constructionLayers.delete("triangles"),selectedMedianChanged=state.constructionLayers.delete("triangle-medians"),selectedBisectorChanged=state.constructionLayers.delete("triangle-perpendicular-bisectors"),visibleTriangleChanged=state.visibleConstructionLayers.delete("triangles"),visibleMedianChanged=state.visibleConstructionLayers.delete("triangle-medians"),visibleBisectorChanged=state.visibleConstructionLayers.delete("triangle-perpendicular-bisectors");if(selectedTriangleChanged||selectedMedianChanged||selectedBisectorChanged||visibleTriangleChanged||visibleMedianChanged||visibleBisectorChanged)syncConstructionVisibility()}
...
function reviewedCandidatesFor(prepared){const saved=publicWidgetState(),patches=saved.reviewedProposalCandidateSetIdentity===prepared.candidateSetIdentity&&Array.isArray(saved.reviewedCandidateGeometry)?saved.reviewedCandidateGeometry:[];return prepared.candidates.map(item=>{const patch=patches.find(value=>value?.id===item.id);if(!validGeometryPatch(patch,item)||!storedRotatedEllipseAdoptionMatches(saved,prepared,item,patch))return JSON.parse(JSON.stringify(item));return{...item,x:patch.x,y:patch.y,width:patch.width,height:patch.height,...(patch.primitive===undefined?{}:{primitive:clonePrimitive(patch.primitive)})}})}
...
const result=payload.result||structured,analysis=payload.imagePlaneGuideAnalysis||structured?.imagePlaneGuideAnalysis||null,confirmedGuideCount=analysis?.confirmedVisualGuideCandidateIds?.length||0,relationCount=analysis?.relationships?.length||0,quadrilateralCount=analysis?.quadrilateralMeasurements?.length||0,constructionCount=(analysis?.constructionAnalysis?.supportLineExtensions?.length||0)+(analysis?.constructionAnalysis?.formatDiagonals?.length||0)+(analysis?.constructionAnalysis?.triangles?.length||0)+(analysis?.constructionAnalysis?.triangleMedians?.length||0),evidenceCount=relationCount+quadrilateralCount+constructionCount;
