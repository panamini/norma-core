...
.shell{container-type:inline-size}.overlay [data-primitive-kind="ellipse"]{touch-action:none}.overlay [data-ellipse-handle="center"]{cursor:move}.overlay [data-ellipse-handle="radius-x"],.overlay [data-ellipse-handle="radius-y"]{cursor:crosshair}
...
function adjustedEllipseCandidate(item,handle,dx,dy){const primitive=item?.primitive;if(primitive?.kind!=="ellipse")return item;const axes=ellipseAxes(primitive);let nextPrimitive=primitive;if(handle==="radius-x"){const delta=dx*axes.x.x+dy*axes.x.y;nextPrimitive={...primitive,radiusX:rounded(Math.max(.005,primitive.radiusX+delta))}}else if(handle==="radius-y"){const delta=dx*axes.y.x+dy*axes.y.y;nextPrimitive={...primitive,radiusY:rounded(Math.max(.005,primitive.radiusY+delta))}}else{const envelope=ellipseEnvelope(primitive),halfWidth=envelope.width/2,halfHeight=envelope.height/2;nextPrimitive={...primitive,center:{x:rounded(Math.max(halfWidth,Math.min(1-halfWidth,primitive.center.x+dx))),y:rounded(Math.max(halfHeight,Math.min(1-halfHeight,primitive.center.y+dy)))}}}const next=candidateWithPrimitive(item,nextPrimitive);return next.x<0||next.y<0||next.x+next.width>1.000001||next.y+next.height>1.000001?item:next}
