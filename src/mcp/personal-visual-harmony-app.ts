...
const activePrepared=restoredPreparedFor(prepared),reviewedCandidates=reviewedCandidatesFor(activePrepared),restoredManual=restoredManualSegmentFor(activePrepared);if(restoredManual)reviewedCandidates.push(restoredManual);initializeGuidedAnalysisForPrepared(prepared,reviewedCandidates);if(state.manualSpatialFallbackSessionId!==state.payload?.sessionId){state.manualSpatialFallback=false;state.manualSpatialFallbackSessionId=null}
