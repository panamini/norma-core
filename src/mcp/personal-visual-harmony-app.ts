...
function bootstrap(){const payload=currentPayload();if(payload){bootstrapRetryCount=0;setWidgetBootstrapState("ready");if(payload.stage==="confirmation_required"&&!state.payload){void hydrate(payload);return}if(payload.stage==="completed"&&!state.completed){void hydrate(payload,window.openai?.toolOutput);return}return}if(bootstrapRetryCount===BOOTSTRAP_PENDING_NOTICE_AFTER){loading.textContent="Connexion au résultat de l’analyse en cours…";setWidgetBootstrapState("stale")}const delay=bootstrapRetryCount<BOOTSTRAP_PENDING_NOTICE_AFTER?BOOTSTRAP_RETRY_DELAY_MS:BOOTSTRAP_SLOW_RETRY_DELAY_MS;bootstrapRetryCount=Math.min(bootstrapRetryCount+1,BOOTSTRAP_PENDING_NOTICE_AFTER);setTimeout(bootstrap,delay)}
