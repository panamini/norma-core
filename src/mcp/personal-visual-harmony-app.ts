...
const PrepareOutputSchema = z.object({
  ...
  automaticHarmonicPreview: z.object({
    contractId: z.literal("norma.personal-visual-harmony-automatic-harmonic-preview@1"),
    ...
    ratioPackRefs: z.array(z.union([
      z.literal("norma.geometry-harmonies@0.1.0"),
      z.literal("norma.basic-proportions@0.1.0"),
    ])).length(2),
    matchTolerance: z.literal(PERSONAL_VISUAL_HARMONY_DECLARED_RATIO_MATCH_TOLERANCE),
    relationships: z.array(z.object({
      relationshipId: z.string().regex(SHA256_PATTERN),
      subjectCandidateId: z.string(),
      relatedCandidateIds: z.array(z.string()).length(1),
      metric: z.enum(["horizontal-split-share", "vertical-split-share"]),
...
.automatic-preview{display:grid;gap:6px;margin:0 12px 10px;padding:10px;border:1px solid var(--rule);background:var(--white)}.automatic-preview[hidden]{display:none}.automatic-preview strong{font-size:12px}.automatic-preview p,.automatic-preview span{margin:0;color:var(--graphite);font-size:10px;line-height:1.4}.automatic-preview ul{display:grid;gap:4px;margin:0;padding-left:17px;font-size:10px;line-height:1.35}.automatic-preview[data-stale="true"]{border-color:var(--proposal)}
...
<main class="shell">
  ...
  <section class="content">
    ...
    <aside id="analysisPanel" class="side" data-analysis-mode="automatic">
      ...
      <section id="automaticPreview" class="automatic-preview" hidden aria-live="polite"></section>
...
function recordReviewEvent(kind,atMs=Date.now(),persist=true){const scope=reviewJournalScope();if(!scope||!REVIEW_EVENT_KINDS.includes(kind)||!Number.isSafeInteger(atMs)||atMs<0)return;if(kind==="candidate-moved"||kind==="candidate-resized"||kind==="candidate-added"||kind==="candidate-removed")invalidateAutomaticHarmonicPreview();const current=storedReviewJournalFor(state.reviewJournal),journal=current||{contractId:REVIEW_JOURNAL_CONTRACT_ID,analysisIdentity:scope.analysisIdentity,fileId:scope.fileId,sessionId:scope.sessionId,prepareDurationMs:reviewJournalPrepareDuration(),events:[]};if(journal.events.length>=MAX_REVIEW_EVENTS)return;state.reviewJournal={...journal,events:[...journal.events,{atMs,kind}]};syncReviewJournalAttributes();if(persist)persistReviewState()}
...
function renderAutomaticHarmonicPreview(){const preview=state.payload?.prepared?.automaticHarmonicPreview,manual=state.guidedAnalysisGoal==="compare-two-lengths";automaticPreview.hidden=manual||!preview;if(automaticPreview.hidden)return;automaticPreview.replaceChildren();automaticPreview.dataset.stale=String(state.automaticPreviewStale);const heading=document.createElement("strong"),note=document.createElement("p");heading.textContent=state.automaticPreviewStale?"Relations candidates à revoir":"Relations candidates à vérifier";note.textContent=state.automaticPreviewStale?"La sélection ou la géométrie a changé : cet aperçu préparatoire est périmé. Core reste arrêté jusqu’à confirmation.":"Proposition préparatoire sur rectangles détectés · aucun provider ni calcul Core n’a été exécuté.";automaticPreview.append(heading,note);if(!state.automaticPreviewStale&&Array.isArray(preview.relationships)&&preview.relationships.length>0){const list=document.createElement("ul");for(const relation of preview.relationships.slice(0,8)){const item=document.createElement("li");item.textContent=relation.subjectCandidateId+" / "+relation.relatedCandidateIds[0]+" · "+relation.ratioLabel+" · "+relation.observedPercent.toFixed(3)+" %";list.append(item)}automaticPreview.append(list)}}
function invalidateAutomaticHarmonicPreview(){if(state.guidedAnalysisGoal==="compare-two-lengths")return;state.automaticPreviewStale=true;renderAutomaticHarmonicPreview()}
function syncAnalysisModeUi(){const manual=state.guidedAnalysisGoal==="compare-two-lengths",spatial=manual&&twoObjectSpatialWorkflowActive();analysisPanel.dataset.analysisMode=manual?"manual":"automatic";flowStepOne.innerHTML=manual?"<b>01</b>Choisir A":"<b>01</b>Explorer";flowStepTwo.innerHTML=manual?"<b>02</b>Choisir B":"<b>02</b>Vérifier";flowStepThree.innerHTML=manual?"<b>03</b>Mesurer":"<b>03</b>Confirmer";workflowHintHeading.textContent=manual?"Comparer deux distances précises":"Explorer les rapports remarquables";workflowHintText.textContent=manual?(spatial?"Cochez deux zones dans la liste; ajustez-les directement sur l’image si nécessaire. Choisissez ensuite la mesure A et la mesure B : les deux traits apparaîtront avant tout calcul.":"Tracez le segment A puis le segment B directement sur l’image. Chaque clic ou glissement définit une extrémité; les deux traits apparaissent avant une confirmation unique."):"Norma part des zones déjà détectées. Vérifiez les guides utiles, puis confirmez une seule fois pour voir les rapports et leur position dans l’image.";if(typeof renderAutomaticHarmonicPreview==="function")renderAutomaticHarmonicPreview();if(!state.completed&&!state.confirming)confirmButton.textContent=manual?"Calculer le rapport A/B":"Analyser les rapports sélectionnés";measurementRatioToggle.hidden=manual}
...
function publicPrepareResult(prepared: PersonalVisualHarmonyPreparedCandidateSet) {
  ...
  return {
    ...
    automaticHarmonicPreview: previewPersonalVisualHarmonyAutomaticHarmonicRelationshipsV1({
