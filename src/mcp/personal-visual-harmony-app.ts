...
const EDIT_HANDLE_HIT_WIDTH=120,EDIT_HANDLE_HIT_HEIGHT=210,EDIT_HANDLE_ATTRIBUTES=["data-resize-handle","data-point-handle","data-vertex-handle","data-ellipse-handle"];
...
function syncHandleHitArea(group,handle){const match=editHandleAttribute(handle);if(!match)return;let hitArea=[...group.querySelectorAll("[data-handle-hit-area]")].find(node=>node.getAttribute(match.attribute)===match.value);if(!hitArea){hitArea=document.createElementNS("http://www.w3.org/2000/svg","rect");hitArea.setAttribute("data-handle-hit-area","true");hitArea.setAttribute(match.attribute,match.value);hitArea.setAttribute("tabindex","-1");hitArea.setAttribute("fill","transparent");hitArea.setAttribute("stroke","none");group.append(hitArea)}const centerX=handle.localName==="circle"?Number(handle.getAttribute("cx")):Number(handle.getAttribute("x"))+Number(handle.getAttribute("width"))/2,centerY=handle.localName==="circle"?Number(handle.getAttribute("cy")):Number(handle.getAttribute("y"))+Number(handle.getAttribute("height"))/2;if(!Number.isFinite(centerX)||!Number.isFinite(centerY))return;const x=Math.max(0,Math.min(1000-EDIT_HANDLE_HIT_WIDTH,centerX-EDIT_HANDLE_HIT_WIDTH/2)),y=Math.max(0,Math.min(1000-EDIT_HANDLE_HIT_HEIGHT,centerY-EDIT_HANDLE_HIT_HEIGHT/2));hitArea.setAttribute("x",String(x));hitArea.setAttribute("y",String(y));hitArea.setAttribute("width",String(EDIT_HANDLE_HIT_WIDTH));hitArea.setAttribute("height",String(EDIT_HANDLE_HIT_HEIGHT))}
function syncAllHandleHitAreas(){for(const group of overlay.querySelectorAll("[data-candidate-id]")){const handles=group.querySelectorAll("[data-resize-handle]:not([data-handle-hit-area]),[data-point-handle]:not([data-handle-hit-area]),[data-vertex-handle]:not([data-handle-hit-area]),[data-ellipse-handle]:not([data-handle-hit-area])");handles.forEach(handle=>syncHandleHitArea(group,handle))}}
