  ...
  server.registerTool(
    ...
    ({
      ...
      const constructionSummary = topConstructions === undefined
        ? ""
        : ` Constructions dérivées optionnelles : ${String(topConstructions.supportLineExtensions.length)} prolongement(s) de droites supports et ${String(topConstructions.formatDiagonals.length)} diagonale(s) du format, séparés des observations et sans autorité Core.`;
      return {
        content: [{
          type: "text" as const,
...
const result=payload.result||structured,analysis=payload.imagePlaneGuideAnalysis||structured?.imagePlaneGuideAnalysis||null,confirmedGuideCount=analysis?.confirmedVisualGuideCandidateIds?.length||0,relationCount=analysis?.relationships?.length||0,quadrilateralCount=analysis?.quadrilateralMeasurements?.length||0,constructionCount=(analysis?.constructionAnalysis?.supportLineExtensions?.length||0)+(analysis?.constructionAnalysis?.formatDiagonals?.length||0),evidenceCount=relationCount+quadrilateralCount+constructionCount;
const completedConstructionLayers=Array.isArray(analysis?.constructionAnalysis?.enabledLayers)?analysis.constructionAnalysis.enabledLayers.filter(layer=>CONSTRUCTION_LAYERS.includes(layer)):[];state.constructionLayers=new Set(completedConstructionLayers);
