...
restoreGuidedAnalysisGoal();renderGuidedAnalysisGoals();
const activePrepared=restoredPreparedFor(prepared);candidateList.replaceChildren();overlay.classList.remove("locked");state.proposalCandidateSetIdentity=prepared.candidateSetIdentity;state.proposalCandidates=prepared.candidates.map(item=>JSON.parse(JSON.stringify(item)));state.reviewedCandidates=reviewedCandidatesFor(activePrepared);const restoredManual=restoredManualSegmentFor(activePrepared);if(restoredManual)state.reviewedCandidates.push(restoredManual);state.manualSegmentCandidateId=state.reviewedCandidates.find(isManualSegmentCandidate)?.id??null;restorePixelRefinementState(activePrepared);
