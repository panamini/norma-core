  ...
  server.registerTool(
    PERSONAL_VISUAL_HARMONY_PREPARE_TOOL,
    {
      ...
      description: [
        ...
        "Quadrilaterals, segments, axes, and axis-aligned ellipses are separately confirmable image-plane guides in this version; the deterministic Core receives confirmed rectangles only, without lossy conversion. Confirmed quadrilaterals expose sides, angles, diagonals, area, and centroid, while ellipse-line and ellipse-quadrilateral-side pairs may be measured for intersection, tangency, or proximity in the image plane. Include at least one defensible rectangle or frame for Core.",
...
function triangleLayerReady(prepared=state.payload?.prepared,layers=state.constructionLayers){const dependencies=triangleRequestDependencies(prepared);return dependencies.length>0&&!geometryChanged()&&dependencies.every(layer=>layers.has(layer))}
...
function restoreConstructionGuideState(prepared){const stored=storedConstructionGuideStateFor(publicWidgetState().constructionGuideState,prepared);state.constructionLayers=new Set(stored?.layers||[]);state.visibleConstructionLayers=new Set(state.constructionLayers);syncConstructionVisibility()}
