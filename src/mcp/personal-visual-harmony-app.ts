  ...
  server.registerTool(
    ...
    ({
      ...
      const constructionSummary = topConstructions === undefined
        ? ""
        : ` Constructions dérivées optionnelles : ${String(topConstructions.supportLineExtensions.length)} prolongement(s) de droites supports, ${String(topConstructions.formatDiagonals.length)} diagonale(s) du format et ${String(topConstructions.triangles?.length ?? 0)} triangle(s) explicitement demandé(s), séparés des observations et sans autorité Core.`;
      return {
        content: [{
          type: "text" as const,
          text: `${structuredContent.headline}${topExplanations.length === 0 ? "" : ` ${topExplanations}`}${topQuadrilateralMeasurements.length === 0 ? "" : ` Mesures de quadrilatères dans le plan image : ${topQuadrilateralMeasurements}`}${topImagePlaneRelations.length === 0 ? "" : ` Relations séparées du plan image : ${topImagePlaneRelations}`}${constructionSummary} Les mesures, relations et constructions de guides ne sont ni des rapports harmoniques ni des mesures du monde réel.`,
        }],
        structuredContent,
        _meta: {
          normaPersonalVisualHarmony: {
...
.guide-marker{display:grid!important;place-items:center;width:18px;height:18px;margin-top:1px!important;color:var(--cyan)!important;font-size:18px!important}.overlay [data-candidate-id]{outline:none}.overlay [data-candidate-id]:focus [data-candidate-shape],.overlay [data-candidate-id]:focus-within [data-candidate-shape]{filter:drop-shadow(0 0 10px #f8fafc)}.construction-controls{display:grid;grid-template-columns:1fr 1fr;gap:6px}.construction-toggle{border:1px solid rgba(34,211,238,.35);border-radius:11px;padding:8px 9px;background:rgba(34,211,238,.08);color:#cffafe;font-size:9px;font-weight:850;cursor:pointer}.construction-toggle[aria-pressed="false"]{border-color:rgba(148,163,184,.2);background:rgba(15,23,42,.4);color:#94a3b8}.construction-toggle:disabled{opacity:.45;cursor:not-allowed}.pixel-toggle{border:1px solid rgba(232,121,249,.4);border-radius:12px;padding:9px 11px;background:rgba(232,121,249,.08);color:#f5d0fe;font-size:10px;font-weight:850;cursor:pointer}.pixel-toggle[aria-pressed="false"]{border-color:rgba(148,163,184,.2);background:rgba(15,23,42,.4);color:#94a3b8}.pixel-evidence{grid-column:1/-1;margin-top:5px;padding-top:7px;border-top:1px solid rgba(148,163,184,.13);color:#cbd5e1;font-size:9px;line-height:1.45}.pixel-evidence button{margin-top:6px;border:1px solid rgba(232,121,249,.45);border-radius:999px;padding:5px 8px;background:rgba(232,121,249,.1);color:#f5d0fe;font-size:9px;font-weight:850;cursor:pointer}.pixel-evidence .identity{display:block;margin-top:4px}.pixel-evidence[data-status="abstained"]{color:#94a3b8}
</style>
<style>
@media(max-width:720px){.shell,.header,.content,.visual,.side,.flow,.construction-controls{width:100%;min-width:0;max-width:100%}.header{align-items:flex-start;flex-wrap:wrap;padding:16px 14px 12px}.brand{flex:1 1 220px;min-width:0}.stage{flex:0 0 auto;margin-left:auto}.content{grid-template-columns:minmax(0,1fr);padding:0 12px 12px}.flow{grid-template-columns:minmax(0,1fr) auto minmax(0,1fr) auto minmax(0,1fr);font-size:9px}.flow span{min-width:0;overflow-wrap:anywhere}.construction-controls{grid-template-columns:minmax(0,1fr)}.construction-toggle,.confirm{width:100%;min-width:0;white-space:normal;overflow-wrap:anywhere}}
</style>
</head>
<body>
<main class="shell">
...
const CONSTRUCTION_LAYERS=["support-line-extensions","format-diagonals","junction-angles","triangles"];
...
function restoreConstructionGuideState(prepared){const stored=storedConstructionGuideStateFor(publicWidgetState().constructionGuideState,prepared);state.constructionLayers=new Set(stored?.layers||[]);state.visibleConstructionLayers=new Set(state.constructionLayers);if(state.constructionLayers.has("triangles")&&!triangleLayerReady(prepared,state.constructionLayers,state.selectedGuides)){state.constructionLayers.delete("triangles");state.visibleConstructionLayers.delete("triangles")}syncConstructionVisibility()}
function toggleConstructionLayer(layer){if(state.confirming||!CONSTRUCTION_LAYERS.includes(layer))return;if(state.completed){if(!state.constructionLayers.has(layer))return;if(state.visibleConstructionLayers.has(layer))state.visibleConstructionLayers.delete(layer);else state.visibleConstructionLayers.add(layer);syncConstructionVisibility();statusNode.textContent=state.visibleConstructionLayers.has(layer)?"Construction dérivée réaffichée. Le résultat confirmé et le Core restent inchangés.":"Construction dérivée masquée. Le résultat confirmé et le Core restent inchangés.";return}if((layer==="junction-angles"&&!state.constructionLayers.has("support-line-extensions"))||(layer==="triangles"&&!state.constructionLayers.has("triangles")&&!triangleLayerReady()))return;if(state.constructionLayers.has(layer)){state.constructionLayers.delete(layer);if(layer==="support-line-extensions")state.constructionLayers.delete("junction-angles")}else state.constructionLayers.add(layer);if(state.constructionLayers.has("triangles")&&!triangleLayerReady())state.constructionLayers.delete("triangles");state.visibleConstructionLayers=new Set(state.constructionLayers);syncConstructionVisibility();persistReviewState();statusNode.textContent=state.constructionLayers.has(layer)?"Construction dérivée affichée. Elle reste séparée des segments observés et hors du Core.":"Construction dérivée masquée. La géométrie observée reste inchangée."}
function invalidateTriangleConstruction(){const selectedChanged=state.constructionLayers.delete("triangles"),visibleChanged=state.visibleConstructionLayers.delete("triangles");if(selectedChanged||visibleChanged)syncConstructionVisibility()}
...
function validPixelProposal(value,prepared){if(!value||typeof value!=="object"||Object.keys(value).sort().join("|")!==["automaticAcceptance","candidateEvidenceOnly","candidateId","candidateSetIdentity","contentIdentity","contractId","contractVersion","coordinateSpace","coreRun","crop","diagnostics","displacementPixels","evidence","explicitProposalAdoptionRequired","explicitUserConfirmationRequired","kernelContentIdentity","originalGeometry","pixelRasterContentIdentity","proposalAdopted","proposedGeometry","reason","sourcePixelHeight","sourcePixelWidth","sourceTruth","status"].sort().join("|")||value.contractId!=="norma.personal-visual-harmony-pixel-refinement-proposal@1"||value.contractVersion!==1||!['refined','abstained'].includes(value.status)||value.candidateSetIdentity!==prepared.candidateSetIdentity||value.candidateEvidenceOnly!==true||value.sourceTruth!==false||value.automaticAcceptance!==false||value.explicitProposalAdoptionRequired!==true||value.proposalAdopted!==false||value.explicitUserConfirmationRequired!==true||value.coreRun!==false||value.coordinateSpace!=="normalized-image"||!Number.isInteger(value.sourcePixelWidth)||value.sourcePixelWidth<8||value.sourcePixelWidth>100000||!Number.isInteger(value.sourcePixelHeight)||value.sourcePixelHeight<8||value.sourcePixelHeight>100000||!isStoredIdentity(value.contentIdentity)||value.pixelRasterContentIdentity!==null&&!isStoredIdentity(value.pixelRasterContentIdentity)||value.kernelContentIdentity!==null&&!isStoredIdentity(value.kernelContentIdentity)||!Array.isArray(value.diagnostics)||value.diagnostics.length<1||value.diagnostics.length>4)return false;const candidate=prepared.candidates.find(item=>item.id===value.candidateId),kind=primitiveKind(candidate);if(!candidate||kind==="rectangle"||JSON.stringify(value.originalGeometry)!==JSON.stringify(candidate.primitive))return false;if(!value.displacementPixels||value.displacementPixels.bound!==6||!Number.isFinite(value.displacementPixels.maximum)||value.displacementPixels.maximum<0||value.displacementPixels.maximum>6.000001||!Number.isFinite(value.displacementPixels.mean)||value.displacementPixels.mean<0||value.displacementPixels.mean>6.000001)return false;if(value.status==="abstained")return value.proposedGeometry===null;if(!value.proposedGeometry||!value.evidence||!Number.isFinite(value.evidence.confidence)||value.evidence.confidence<0||value.evidence.confidence>1)return false;try{return validGeometryPatch(geometrySnapshotFor(candidateWithPrimitive(candidate,clonePrimitive(value.proposedGeometry),true)),candidate)}catch{return false}}
...
function ellipseEnvelope(primitive){const rotation=(primitive.rotationDegrees||0)*Math.PI/180,cos=Math.cos(rotation),sin=Math.sin(rotation),halfWidth=Math.hypot(primitive.radiusX*cos,primitive.radiusY*sin),halfHeight=Math.hypot(primitive.radiusX*sin,primitive.radiusY*cos);return{x:primitive.center.x-halfWidth,y:primitive.center.y-halfHeight,width:halfWidth*2,height:halfHeight*2}}
...
function validGeometryPatch(value,candidate){if(!value||value.id!==candidate.id||!Number.isFinite(value.x)||!Number.isFinite(value.y)||!Number.isFinite(value.width)||!Number.isFinite(value.height)||value.x<0||value.y<0||value.width<0||value.height<0||value.x+value.width>1.000001||value.y+value.height>1.000001)return false;const kind=primitiveKind(candidate),primitive=value.primitive,expectedKeys=(candidate.primitive===undefined?["height","id","width","x","y"]:["height","id","primitive","width","x","y"]).sort().join("|");if(Object.keys(value).sort().join("|")!==expectedKeys)return false;if(kind==="rectangle")return value.width>0&&value.height>0&&(candidate.primitive===undefined||primitive?.kind==="rectangle");if(!primitive||primitive.kind!==kind)return false;if(kind==="segment"||kind==="axis")return Object.keys(primitive).sort().join("|")==="end|kind|start"&&validPoint(primitive.start)&&validPoint(primitive.end)&&(primitive.start.x!==primitive.end.x||primitive.start.y!==primitive.end.y)&&envelopeMatches(value,pointEnvelope([primitive.start,primitive.end]));if(kind==="quadrilateral")return Object.keys(primitive).sort().join("|")==="kind|vertices"&&validQuadrilateralVertices(primitive.vertices)&&envelopeMatches(value,pointEnvelope(primitive.vertices));if(kind==="ellipse"){const fields=Object.keys(primitive).sort().join("|");if(candidate.primitive?.rotationDegrees!==undefined&&JSON.stringify(primitive)!==JSON.stringify(candidate.primitive))return false;return(fields==="center|kind|radiusX|radiusY"||fields==="center|kind|radiusX|radiusY|rotationDegrees")&&validPoint(primitive.center)&&Number.isFinite(primitive.radiusX)&&Number.isFinite(primitive.radiusY)&&primitive.radiusX>0&&primitive.radiusY>0&&(primitive.rotationDegrees===undefined||Number.isFinite(primitive.rotationDegrees))&&envelopeMatches(value,ellipseEnvelope(primitive))}return false}
function reviewedCandidatesFor(prepared){const saved=publicWidgetState(),patches=saved.reviewedProposalCandidateSetIdentity===prepared.candidateSetIdentity&&Array.isArray(saved.reviewedCandidateGeometry)?saved.reviewedCandidateGeometry:[];return prepared.candidates.map(item=>{const patch=patches.find(value=>value?.id===item.id);if(!validGeometryPatch(patch,item))return JSON.parse(JSON.stringify(item));return{...item,x:patch.x,y:patch.y,width:patch.width,height:patch.height,...(patch.primitive===undefined?{}:{primitive:clonePrimitive(patch.primitive)})}})}
function persistReviewState(){window.openai?.setWidgetState?.({...publicWidgetState(),selectedCandidateIds:coreSelectedIds(),confirmedVisualGuideCandidateIds:confirmedGuideIds(),reviewedProposalCandidateSetIdentity:state.proposalCandidateSetIdentity,reviewedCandidateGeometry:geometrySnapshot(),constructionGuideState:constructionLayerSnapshot(),pixelRefinementState:pixelRefinementSnapshot()})}
function persistSelection(){persistReviewState()}
function geometryChanged(candidates=state.reviewedCandidates){return candidates.some((item,index)=>{const original=state.proposalCandidates[index];return !original||JSON.stringify(geometrySnapshotFor(item))!==JSON.stringify(geometrySnapshotFor(original))})}
...
function svgPointHandle(attribute,value){const handle=document.createElementNS("http://www.w3.org/2000/svg","circle");handle.setAttribute(attribute,String(value));handle.setAttribute("tabindex","0");handle.setAttribute("r","15");handle.setAttribute("fill","#f8fafc");handle.setAttribute("stroke","#0f172a");handle.setAttribute("stroke-width","5");return handle}
...
function updateConfirm(){const noCoreRectangle=coreSelectedIds().length===0;confirmButton.disabled=state.completed||state.confirming||state.pixelRefinementRunning||!state.imageReady||noCoreRectangle||!state.payload;if(!state.completed&&!state.confirming&&state.imageReady&&noCoreRectangle)statusNode.textContent="Sélectionnez au moins un rectangle structurel pour lancer le Core actuel."}
function setReviewLocked(locked){const disabled=locked||state.completed;overlay.classList.toggle("locked",disabled);candidateList.querySelectorAll("input").forEach(input=>input.disabled=disabled);overlay.querySelectorAll("[data-candidate-id]").forEach(group=>{const editable=group.getAttribute("data-primitive-kind")!=="ellipse"&&!disabled;group.setAttribute("tabindex",editable?"0":"-1");group.querySelectorAll("[data-resize-handle],[data-point-handle],[data-vertex-handle]").forEach(handle=>handle.setAttribute("tabindex",editable?"0":"-1"));if(disabled)group.setAttribute("aria-disabled","true");else group.removeAttribute("aria-disabled")});updateConstructionControls();updatePixelProposalUi();updateConfirm()}
...
if(persist&&state.payload?.prepared?.candidateSetIdentity&&state.dimensions){const reviewedCandidateGeometry=geometrySnapshot(),candidateSetIdentity=state.proposalCandidateSetIdentity||state.payload.prepared.candidateSetIdentity,selectedCandidateIds=coreSelectedIds(),confirmedVisualGuideCandidateIds=confirmedGuideIds(),constructionGuideState=constructionLayerSnapshot(),pixelRefinementState=pixelRefinementSnapshot();window.openai?.setWidgetState?.({...publicWidgetState(),selectedCandidateIds,confirmedVisualGuideCandidateIds,reviewedProposalCandidateSetIdentity:candidateSetIdentity,reviewedCandidateGeometry,constructionGuideState,pixelRefinementState,completedVisualHarmony:{operation:CONFIRM_TOOL,candidateSetIdentity,reviewedCandidateGeometry,constructionGuideState,pixelRefinementState,selectedCandidateIds,confirmedVisualGuideCandidateIds,sourcePixelWidth:state.dimensions.width,sourcePixelHeight:state.dimensions.height,confirmedSelectionIdentity:result.confirmedSelectionIdentity||"",mappedGeometryContentIdentity:result.mappedGeometryContentIdentity||structured?.mappedGeometryContentIdentity||"",imagePlaneGuideAnalysisContentIdentity:analysis?.contentIdentity||undefined,ratioPackRefs:structured?.ratioPackRefs||[],headline:result.headline||structured?.headline||"Analyse terminée",canonicalResultIdentity,matches:explanations.map(item=>({subjectCandidateId:item.subjectCandidateId,subjectLabel:item.subjectLabel,relatedCandidateIds:item.relatedCandidateIds,metric:item.metric,ratioLabel:item.ratioLabel,observedPercent:item.observedPercent,targetPercent:item.targetPercent,deltaPercentagePoints:item.deltaPercentagePoints})),presentation}})}updateConstructionControls();updatePixelProposalUi();updateConfirm()}
...
async function callConfirmation(payload,selectedCandidateIds,confirmedVisualGuideCandidateIds,constructionLayers,dimensions){const args={sessionId:payload.sessionId,candidateSetIdentity:payload.prepared.candidateSetIdentity,selectedCandidateIds,confirmedVisualGuideCandidateIds,constructionLayers,sourcePixelWidth:dimensions.width,sourcePixelHeight:dimensions.height,confirmClientReviewedSelection:true,recovery:pixelRecovery(payload)};return callAppTool(CONFIRM_TOOL,args)}
...
async function hydrate(payload=currentPayload(),structured=window.openai?.toolOutput,{forceImageReload=false}={}){if(!payload)return;const identity=payloadIdentity(payload);state.pendingStructuredContent=structured;state.activePayload=payload;state.activePayloadIdentity=identity;if(payload.stage==="confirmation_required"&&state.confirming){state.payload=payload;return}if(payload.stage==="confirmation_required"||!state.payload||state.payload.fileId!==payload.fileId)state.payload=payload;if(payload.overlaySvg)overlay.innerHTML=safeSvg(payload.overlaySvg);if(payload.stage==="confirmation_required"){renderCandidates(payload.prepared);const imageLoaded=await loadImage(payload.fileId,identity,{force:forceImageReload});if(!imageLoaded||state.activePayloadIdentity!==identity)return;if(state.pixelRefinementEnabled)await refreshPixelRefinements(payload,identity);if(state.activePayloadIdentity!==identity)return;const completed=completedWidgetStateFor(payload);if(completed&&!state.confirming&&!state.completed)await revalidateCompleted(payload,completed,identity);return}if(payload.stage==="completed"){if(payload.fileId&&!await loadImage(payload.fileId,identity,{force:forceImageReload}))return;if(state.activePayloadIdentity!==identity)return;renderResult(payload,structured)}}
confirmButton.addEventListener("click",async()=>{if(state.confirming||state.pixelRefinementRunning||!state.payload||!state.dimensions||coreSelectedIds().length===0)return;state.confirming=true;const payloadSnapshot=state.payload,payloadIdentitySnapshot=state.activePayloadIdentity,candidateSnapshot=reviewedCandidateSnapshot(),selectedSnapshot=Object.freeze(coreSelectedIds()),guideSnapshot=Object.freeze(confirmedGuideIds()),constructionSnapshot=Object.freeze(CONSTRUCTION_LAYERS.filter(layer=>state.constructionLayers.has(layer))),dimensionsSnapshot=Object.freeze({...state.dimensions}),changed=geometryChanged(candidateSnapshot);setReviewLocked(true);confirmButton.textContent="Norma mesure…";statusNode.textContent=changed?"Corrections structurées en cours de validation avant mesure…":"Sélection confirmée dans le widget. Calcul du Core et des relations du plan image…";try{const analysisPayload=changed?await prepareReviewedPayload(payloadSnapshot,candidateSnapshot):payloadSnapshot;const response=await callConfirmation(analysisPayload,selectedSnapshot,guideSnapshot,constructionSnapshot,dimensionsSnapshot);if(state.activePayloadIdentity!==payloadIdentitySnapshot)return;const structured=findCompletedResult(response);const hiddenPayload=findPayload(response);if(!structured)throw new Error("missing verified result");state.reviewedCandidates=candidateSnapshot.map(item=>({...item}));state.selected=new Set(selectedSnapshot);state.selectedGuides=new Set(guideSnapshot);state.constructionLayers=new Set(constructionSnapshot);state.dimensions={...dimensionsSnapshot};const completedPayload=hiddenPayload||{stage:"completed",result:structured,imagePlaneGuideAnalysis:structured.imagePlaneGuideAnalysis,overlaySvg:""};renderResult(completedPayload,structured);await sendCompletionFollowUp(completedPayload,structured)}catch{if(state.activePayloadIdentity!==payloadIdentitySnapshot)return;statusNode.textContent="Analyse interrompue : les corrections n’ont pas pu être validées par le connecteur local. Réessayez depuis cette image.";confirmButton.textContent="Réessayer l’analyse"}finally{finishConfirmingPayload(payloadIdentitySnapshot)}});
