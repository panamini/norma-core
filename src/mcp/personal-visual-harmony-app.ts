...
function invalidateAutomaticHarmonicPreviewForKind(kind){if(kind==="rectangle")invalidateAutomaticHarmonicPreview()}
function reviewedRectangleGeometryChanged(){const originalById=new Map(state.proposalCandidates.filter(item=>primitiveKind(item)==="rectangle").map(item=>[item.id,item])),reviewed=state.reviewedCandidates.filter(item=>primitiveKind(item)==="rectangle");return reviewed.length!==originalById.size||reviewed.some(item=>{const original=originalById.get(item.id);return !original||JSON.stringify(geometrySnapshotFor(item))!==JSON.stringify(geometrySnapshotFor(original))})}
function invalidateTriangleConstruction(){if(!state.automaticPreviewStale&&reviewedRectangleGeometryChanged())invalidateAutomaticHarmonicPreviewForKind("rectangle");let changed=false;for(const layer of ["triangles","triangle-medians","triangle-perpendicular-bisectors","triangle-angle-bisectors","triangle-altitudes","triangle-centroids"]){changed=state.constructionLayers.delete(layer)||changed;changed=state.visibleConstructionLayers.delete(layer)||changed}if(changed)syncConstructionVisibility()}
