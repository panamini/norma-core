...
const GUIDED_ANALYSIS_GOALS=${JSON.stringify(PERSONAL_VISUAL_HARMONY_GUIDED_ANALYSIS_GOALS_V1)},DEFAULT_GUIDED_ANALYSIS_GOAL="general-geometry";
...
function applyGuidedAnalysisGoal(id){const goal=GUIDED_ANALYSIS_GOALS.find(value=>value.id===id)||GUIDED_ANALYSIS_GOALS[0];state.guidedAnalysisGoal=goal.id;state.visibleKinds=new Set(goal.visibleKinds);updateGuidedAnalysisGoalButtons();updateFamilyFilterButtons();syncFamilyVisibility();guidedGoalStatus.textContent=goal.effect;persistReviewState()}
function restoreGuidedAnalysisGoal(){const stored=publicWidgetState().guidedAnalysisGoal,goal=GUIDED_ANALYSIS_GOALS.find(value=>value.id===stored)||GUIDED_ANALYSIS_GOALS[0];state.guidedAnalysisGoal=goal.id;state.visibleKinds=new Set(goal.visibleKinds)}
function renderGuidedAnalysisGoals(){guidedGoals.replaceChildren();for(const goal of GUIDED_ANALYSIS_GOALS){const button=document.createElement("button");button.type="button";button.className="guided-goal";button.setAttribute("data-goal-id",goal.id);button.setAttribute("aria-pressed",String(goal.id===state.guidedAnalysisGoal));const title=document.createElement("strong"),effect=document.createElement("span");title.textContent=goal.label;effect.textContent=goal.effect;button.append(title,effect);button.addEventListener("click",()=>applyGuidedAnalysisGoal(goal.id));guidedGoals.append(button)}updateGuidedAnalysisGoalButtons();guidedGoalStatus.textContent=GUIDED_ANALYSIS_GOALS.find(value=>value.id===state.guidedAnalysisGoal)?.effect||GUIDED_ANALYSIS_GOALS[0].effect}
function renderFamilyFilters(prepared){familyFilters.replaceChildren();const candidates=state.reviewedCandidates.length>0?state.reviewedCandidates:prepared.candidates,kinds=[...new Set(candidates.map(primitiveKind))];for(const kind of kinds){const button=document.createElement("button");button.type="button";button.className="family-filter";button.textContent=primitiveLabel(kind);button.setAttribute("data-primitive-kind",kind);button.setAttribute("aria-pressed",String(state.visibleKinds.has(kind)));button.addEventListener("click",()=>{if(state.visibleKinds.has(kind))state.visibleKinds.delete(kind);else state.visibleKinds.add(kind);button.setAttribute("aria-pressed",String(state.visibleKinds.has(kind)));syncFamilyVisibility()});familyFilters.append(button)}}
