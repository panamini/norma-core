...
function updateSpatialRecoveryUi(){if(!spatialRecovery)return;const manual=spatialWorkflowBinding().status==="manual",required=spatialRecoveryRequired(),busy=state.spatialRecoveryRunning||state.confirming||state.perceptionRunning||state.completed;spatialRecovery.hidden=!required&&!manual;spatialRecoveryActions.hidden=manual;spatialRecoveryStatus.textContent=manual?"MODE MANUEL · preuves non liées · aucune liaison SAM A/B. Les deux rectangles restent des candidats explicitement sélectionnés.":"La revue SAM A/B est interrompue. Choisissez explicitement une nouvelle session SAM ou un repli manuel non lié.";restartSpatialReview.disabled=!required||busy;continueManualSpatial.disabled=!required||busy||manualSelectedRectangleIds().length!==2;continueManualSpatial.title=manualSelectedRectangleIds().length===2?"":"Sélectionnez exactement deux rectangles avant le repli manuel."}
async function runSpatialRecovery(manual){const payload=state.payload;if(!payload?.prepared||!spatialRecoveryRequired()||state.spatialRecoveryRunning||manual&&manualSelectedRectangleIds().length!==2)return;const candidateSnapshot=spatialRecoveryCandidateSnapshot(payload,manual),selectedSnapshot=manualSelectedRectangleIds(),reuseCurrentManual=manual&&payload.prepared.contractVersion===1&&!state.perceptionReconciliationBlocked;state.spatialRecoveryRunning=true;setReviewLocked(true);let successMessage=null;try{const fresh=reuseCurrentManual?payload:await prepareSpatialRecoveryPayload(payload,candidateSnapshot);state.perceptionReconciliationBlocked=false;state.multiPerceptionTerminalState=null;state.guidedAnalysisGoal="compare-two-lengths";state.measurementRatioRefs=[];if(!reuseCurrentManual)await hydrate(fresh);state.guidedAnalysisGoal="compare-two-lengths";state.manualSpatialFallback=manual;state.manualSpatialFallbackSessionId=manual?fresh.sessionId:null;state.measurementRatioEnabled=manual;if(manual)state.selected=new Set(selectedSnapshot);renderGuidedAnalysisGoals();updatePerceptionUi();updateMeasurementRatioControls();persistReviewState();successMessage=manual?"Comparaison manuelle activée avec exactement deux rectangles explicitement sélectionnés. Preuves non liées, sans liaison SAM A/B; Core reste arrêté.":"Nouvelle session SAM A/B prête. Cliquez explicitement sur « Proposer l’objet A »; aucun provider ni Core n’a été lancé automatiquement."}catch{successMessage="La récupération n’a pas pu créer une nouvelle session. La revue reste arrêtée; réessayez explicitement."}finally{state.spatialRecoveryRunning=false;setReviewLocked(multiPerceptionReviewLocked());updateSpatialRecoveryUi();if(successMessage!==null)statusNode.textContent=successMessage}}
