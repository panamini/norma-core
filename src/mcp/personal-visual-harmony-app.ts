...
<main class="shell">
  ...
  <section class="content">
    ...
    <aside class="side">
      ...
      <div class="construction-controls" aria-label="Constructions dérivées optionnelles"><button id="supportLineToggle" class="construction-toggle" type="button" aria-pressed="false">Prolongements · masqués</button><button id="formatDiagonalToggle" class="construction-toggle" type="button" aria-pressed="false">Diagonales format · masquées</button><button id="junctionAngleToggle" class="construction-toggle" type="button" aria-pressed="false" disabled>Angles jonction · masqués</button><button id="triangleToggle" class="construction-toggle" type="button" aria-pressed="false" disabled>Triangles · masqués</button><button id="triangleMedianToggle" class="construction-toggle" type="button" aria-pressed="false" disabled>Médianes · masquées</button></div>
...
const CONSTRUCTION_LAYERS=["support-line-extensions","format-diagonals","junction-angles","triangles","triangle-medians"];
...
function restoreConstructionGuideState(prepared){const stored=storedConstructionGuideStateFor(publicWidgetState().constructionGuideState,prepared);state.constructionLayers=new Set(stored?.layers||[]);state.visibleConstructionLayers=new Set(state.constructionLayers);if(state.constructionLayers.has("triangles")&&!triangleLayerReady(prepared,state.constructionLayers,state.selectedGuides)){state.constructionLayers.delete("triangles");state.constructionLayers.delete("triangle-medians");state.visibleConstructionLayers.delete("triangles");state.visibleConstructionLayers.delete("triangle-medians")}syncConstructionVisibility()}
function toggleConstructionLayer(layer){if(state.confirming||!CONSTRUCTION_LAYERS.includes(layer))return;if(state.completed){if(!state.constructionLayers.has(layer))return;if(state.visibleConstructionLayers.has(layer))state.visibleConstructionLayers.delete(layer);else state.visibleConstructionLayers.add(layer);syncConstructionVisibility();statusNode.textContent=state.visibleConstructionLayers.has(layer)?"Construction dérivée réaffichée. Le résultat confirmé et le Core restent inchangés.":"Construction dérivée masquée. Le résultat confirmé et le Core restent inchangés.";return}if((layer==="junction-angles"&&!state.constructionLayers.has("support-line-extensions"))||(layer==="triangles"&&!state.constructionLayers.has("triangles")&&!triangleLayerReady())||(layer==="triangle-medians"&&!state.constructionLayers.has("triangle-medians")&&!triangleMedianLayerReady()))return;if(state.constructionLayers.has(layer)){state.constructionLayers.delete(layer);if(layer==="support-line-extensions"){state.constructionLayers.delete("junction-angles");state.constructionLayers.delete("triangles");state.constructionLayers.delete("triangle-medians")}if(layer==="triangles")state.constructionLayers.delete("triangle-medians")}else state.constructionLayers.add(layer);if(state.constructionLayers.has("triangles")&&!triangleLayerReady()){state.constructionLayers.delete("triangles");state.constructionLayers.delete("triangle-medians")}if(state.constructionLayers.has("triangle-medians")&&!triangleMedianLayerReady())state.constructionLayers.delete("triangle-medians");state.visibleConstructionLayers=new Set(state.constructionLayers);syncConstructionVisibility();persistReviewState();statusNode.textContent=state.constructionLayers.has(layer)?"Construction dérivée affichée. Elle reste séparée des segments observés et hors du Core.":"Construction dérivée masquée. La géométrie observée reste inchangée."}
function invalidateTriangleConstruction(){const selectedTriangleChanged=state.constructionLayers.delete("triangles"),selectedMedianChanged=state.constructionLayers.delete("triangle-medians"),visibleTriangleChanged=state.visibleConstructionLayers.delete("triangles"),visibleMedianChanged=state.visibleConstructionLayers.delete("triangle-medians");if(selectedTriangleChanged||selectedMedianChanged||visibleTriangleChanged||visibleMedianChanged)syncConstructionVisibility()}
