...
function persistReviewState(){window.openai?.setWidgetState?.({...publicWidgetState(),selectedCandidateIds:coreSelectedIds(),confirmedVisualGuideCandidateIds:confirmedGuideIds(),reviewedProposalCandidateSetIdentity:state.proposalCandidateSetIdentity,reviewedCandidateGeometry:geometrySnapshot()})}
...
if(persist&&state.payload?.prepared?.candidateSetIdentity&&state.dimensions){const reviewedCandidateGeometry=geometrySnapshot(),candidateSetIdentity=state.proposalCandidateSetIdentity||state.payload.prepared.candidateSetIdentity,selectedCandidateIds=coreSelectedIds(),confirmedVisualGuideCandidateIds=confirmedGuideIds();window.openai?.setWidgetState?.({...publicWidgetState(),selectedCandidateIds,confirmedVisualGuideCandidateIds,reviewedProposalCandidateSetIdentity:candidateSetIdentity,reviewedCandidateGeometry,completedVisualHarmony:{operation:CONFIRM_TOOL,candidateSetIdentity,reviewedCandidateGeometry,selectedCandidateIds,confirmedVisualGuideCandidateIds,sourcePixelWidth:state.dimensions.width,sourcePixelHeight:state.dimensions.height,confirmedSelectionIdentity:result.confirmedSelectionIdentity||"",mappedGeometryContentIdentity:result.mappedGeometryContentIdentity||structured?.mappedGeometryContentIdentity||"",imagePlaneGuideAnalysisContentIdentity:analysis?.contentIdentity||undefined,ratioPackRefs:structured?.ratioPackRefs||[],headline:result.headline||structured?.headline||"Analyse terminée",canonicalResultIdentity,matches:explanations.map(item=>({subjectCandidateId:item.subjectCandidateId,subjectLabel:item.subjectLabel,metric:item.metric,ratioLabel:item.ratioLabel,observedPercent:item.observedPercent,targetPercent:item.targetPercent,deltaPercentagePoints:item.deltaPercentagePoints})),presentation}})}updateConfirm()}
