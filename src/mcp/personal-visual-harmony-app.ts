...
.overlay [data-handle-hit-area]{fill:transparent;pointer-events:none}.overlay [data-handle-hit-area][data-resize-handle]{cursor:nwse-resize}.overlay [data-handle-hit-area][data-point-handle],.overlay [data-handle-hit-area][data-vertex-handle],.overlay [data-handle-hit-area][data-ellipse-handle="radius-x"],.overlay [data-handle-hit-area][data-ellipse-handle="radius-y"]{cursor:crosshair}.overlay [data-handle-hit-area][data-ellipse-handle="center"]{cursor:move}@media(max-width:720px){.overlay [data-handle-hit-area]{pointer-events:all}}
</style>
<style>
...
<script type="module">
const PREPARE_TOOL=${JSON.stringify(PERSONAL_VISUAL_HARMONY_PREPARE_TOOL)},CONFIRM_TOOL=${JSON.stringify(PERSONAL_VISUAL_HARMONY_CONFIRM_TOOL)},REFINE_PIXELS_TOOL=${JSON.stringify(PERSONAL_VISUAL_HARMONY_REFINE_PIXELS_TOOL)},OBSERVABILITY_CONTRACT_ID=${JSON.stringify(PERSONAL_VISUAL_HARMONY_OBSERVABILITY_CONTRACT_ID)};
...
const EDIT_HANDLE_HIT_WIDTH=120,EDIT_HANDLE_HIT_HEIGHT=210,EDIT_HANDLE_ATTRIBUTES=["data-resize-handle","data-point-handle","data-vertex-handle","data-ellipse-handle"],EDIT_HANDLE_SELECTOR="[data-resize-handle]:not([data-handle-hit-area]),[data-point-handle]:not([data-handle-hit-area]),[data-vertex-handle]:not([data-handle-hit-area]),[data-ellipse-handle]:not([data-handle-hit-area])";
...
function matchingHandleHitArea(group,handle){const match=editHandleAttribute(handle);return match?[...group.querySelectorAll("[data-handle-hit-area]")].find(node=>node.getAttribute(match.attribute)===match.value)||null:null}
...
function resolveEditablePointerTarget(group,target,event,svg){const visible=target.closest(EDIT_HANDLE_SELECTOR);if(visible)return visible;if(!target.closest("[data-handle-hit-area]"))return null;const bounds=svg.getBoundingClientRect();if(!(bounds.width>0&&bounds.height>0))return null;const point={x:(event.clientX-bounds.left)*1000/bounds.width,y:(event.clientY-bounds.top)*1000/bounds.height};return[...group.querySelectorAll(EDIT_HANDLE_SELECTOR)].map((handle,index)=>({handle,index,hitArea:matchingHandleHitArea(group,handle),center:editableHandleCenter(handle)})).filter(candidate=>candidate.hitArea&&hitAreaContains(candidate.hitArea,point)&&Number.isFinite(candidate.center.x)&&Number.isFinite(candidate.center.y)).sort((left,right)=>{const leftDistance=(left.center.x-point.x)**2+(left.center.y-point.y)**2,rightDistance=(right.center.x-point.x)**2+(right.center.y-point.y)**2;return leftDistance-rightDistance||left.index-right.index})[0]?.handle||null}
function syncHandleHitArea(group,handle){const match=editHandleAttribute(handle);if(!match)return;let hitArea=matchingHandleHitArea(group,handle);if(!hitArea){hitArea=document.createElementNS("http://www.w3.org/2000/svg","rect");hitArea.setAttribute("data-handle-hit-area","true");hitArea.setAttribute(match.attribute,match.value);hitArea.setAttribute("tabindex","-1");hitArea.setAttribute("fill","transparent");hitArea.setAttribute("stroke","none");group.insertBefore(hitArea,handle)}const center=editableHandleCenter(handle);if(!Number.isFinite(center.x)||!Number.isFinite(center.y))return;const x=Math.max(0,Math.min(1000-EDIT_HANDLE_HIT_WIDTH,center.x-EDIT_HANDLE_HIT_WIDTH/2)),y=Math.max(0,Math.min(1000-EDIT_HANDLE_HIT_HEIGHT,center.y-EDIT_HANDLE_HIT_HEIGHT/2));hitArea.setAttribute("x",String(x));hitArea.setAttribute("y",String(y));hitArea.setAttribute("width",String(EDIT_HANDLE_HIT_WIDTH));hitArea.setAttribute("height",String(EDIT_HANDLE_HIT_HEIGHT))}
function syncAllHandleHitAreas(){for(const group of overlay.querySelectorAll("[data-candidate-id]")){const handles=group.querySelectorAll(EDIT_HANDLE_SELECTOR);handles.forEach(handle=>syncHandleHitArea(group,handle))}}
const handleHitAreaObserver=new MutationObserver(mutations=>{if(mutations.some(mutation=>mutation.type==="childList"||!mutation.target.closest("[data-handle-hit-area]")))syncAllHandleHitAreas()});handleHitAreaObserver.observe(overlay,{attributes:true,attributeFilter:["cx","cy","x","y","width","height"],childList:true,subtree:true});
