...
function spatialRecoveryRequired(){if(state.guidedAnalysisGoal!=="compare-two-lengths"||state.completed)return false;const binding=spatialWorkflowBinding(),unboundUnavailable=binding.status==="unbound"&&perceptionToggle.hidden;return state.multiPerceptionTerminalState!==null||state.perceptionReconciliationBlocked||binding.status==="invalid"||unboundUnavailable}
