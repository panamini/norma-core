...
function guidedAnalysisScope(){const payload=state.activePayload,fileId=payload?.fileId,analysisIdentity=payload?.stage==="confirmation_required"?(state.proposalCandidateSetIdentity||payload?.prepared?.candidateSetIdentity):payload?.result?.contentIdentity||payload?.result?.canonicalResultIdentity||payload?.canonicalResultIdentity;return typeof fileId==="string"&&fileId.length>0&&typeof analysisIdentity==="string"&&analysisIdentity.length>0?{analysisIdentity,fileId}:null}
function visibleKindsForGuidedAnalysisGoal(goal){if(goal.id!=="triangles-constructions")return[...goal.visibleKinds];const prepared=state.activePayload?.prepared||state.payload?.prepared,parentIds=new Set(triangleRequestParentGuideIds(prepared)),candidates=state.reviewedCandidates.length>0?state.reviewedCandidates:(prepared?.candidates||[]),parentKinds=new Set(candidates.filter(item=>parentIds.has(item.id)).map(primitiveKind));return parentKinds.size>0?GUIDED_ANALYSIS_KINDS.filter(kind=>parentKinds.has(kind)):[...goal.visibleKinds]}
...
function renderGuidedAnalysisGoals(){guidedGoals.replaceChildren();for(const goal of GUIDED_ANALYSIS_GOALS){const button=document.createElement("button");button.type="button";button.className="guided-goal";button.setAttribute("data-goal-id",goal.id);button.setAttribute("aria-pressed",String(goal.id===state.guidedAnalysisGoal));const title=document.createElement("strong"),effect=document.createElement("span");title.textContent=goal.label;effect.textContent=goal.effect;button.append(title,effect);button.addEventListener("click",()=>applyGuidedAnalysisGoal(goal.id));guidedGoals.append(button)}updateGuidedAnalysisGoalButtons();guidedGoalStatus.textContent=GUIDED_ANALYSIS_GOALS.find(value=>value.id===state.guidedAnalysisGoal)?.effect||CUSTOM_GUIDED_ANALYSIS_GOAL_EFFECT}
function toggleFamilyVisibility(kind){if(!GUIDED_ANALYSIS_KINDS.includes(kind))return;if(state.visibleKinds.has(kind))state.visibleKinds.delete(kind);else state.visibleKinds.add(kind);state.guidedAnalysisGoal=null;updateGuidedAnalysisGoalButtons();updateFamilyFilterButtons();syncFamilyVisibility();guidedGoalStatus.textContent=CUSTOM_GUIDED_ANALYSIS_GOAL_EFFECT;persistGuidedAnalysisGoal()}
function renderFamilyFilters(prepared){familyFilters.replaceChildren();const candidates=state.reviewedCandidates.length>0?state.reviewedCandidates:prepared.candidates,kinds=[...new Set(candidates.map(primitiveKind))];for(const kind of kinds){const button=document.createElement("button");button.type="button";button.className="family-filter";button.textContent=primitiveLabel(kind);button.setAttribute("data-primitive-kind",kind);button.setAttribute("aria-pressed",String(state.visibleKinds.has(kind)));button.addEventListener("click",()=>toggleFamilyVisibility(kind));familyFilters.append(button)}}
...
function initializeGuidedAnalysisForPrepared(prepared){state.proposalCandidateSetIdentity=prepared.candidateSetIdentity;restoreGuidedAnalysisGoal();renderGuidedAnalysisGoals()}
function renderCandidates(prepared){
initializeGuidedAnalysisForPrepared(prepared);
const activePrepared=restoredPreparedFor(prepared);candidateList.replaceChildren();overlay.classList.remove("locked");state.proposalCandidates=prepared.candidates.map(item=>JSON.parse(JSON.stringify(item)));state.reviewedCandidates=reviewedCandidatesFor(activePrepared);const restoredManual=restoredManualSegmentFor(activePrepared);if(restoredManual)state.reviewedCandidates.push(restoredManual);state.manualSegmentCandidateId=state.reviewedCandidates.find(isManualSegmentCandidate)?.id??null;restorePixelRefinementState(activePrepared);
