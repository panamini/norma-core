...
const activePrepared=restoredPreparedFor(prepared);candidateList.replaceChildren();overlay.classList.remove("locked");state.proposalCandidateSetIdentity=activePrepared.candidateSetIdentity;state.proposalCandidates=activePrepared.candidates.map(item=>JSON.parse(JSON.stringify(item)));state.reviewedCandidates=reviewedCandidatesFor(activePrepared);const restoredManual=restoredManualSegmentFor(activePrepared);if(restoredManual)state.reviewedCandidates.push(restoredManual);state.manualSegmentCandidateId=state.reviewedCandidates.find(isManualSegmentCandidate)?.id??null;if(state.payload?.prepared)state.payload={...state.payload,prepared:activePrepared};restorePixelRefinementState(activePrepared);
