...
const ImagePlaneRelationSchema = z.object({
  ...
  lineLabel: z.string(),
  linePrimitiveKind: z.enum(["segment", "axis"]),
  ...
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions: [
        ...
        "The current deterministic Core mapping measures confirmed rectangles only. Segments, axes, and axis-aligned ellipses remain separately confirmed image-plane guides and must never be silently converted into rectangles; confirmed ellipse-line pairs may yield deterministic intersection, tangency, or proximity evidence without becoming harmonic Core claims.",
  ...
  server.registerTool(
    PERSONAL_VISUAL_HARMONY_PREPARE_TOOL,
    {
      ...
      description: [
        ...
        "Segments, axes, and axis-aligned ellipses are separately confirmable image-plane guides in this version; the deterministic Core receives confirmed rectangles only, without lossy conversion. Confirmed ellipse-line pairs may be measured for intersection, tangency, or proximity in the image plane. Include at least one defensible rectangle or frame for Core.",
  ...
  server.registerTool(
    ...
    ({
      ...
      return {
        content: [{
          type: "text" as const,
          text: `${structuredContent.headline}${topExplanations.length === 0 ? "" : ` ${topExplanations}`}${topImagePlaneRelations.length === 0 ? "" : ` Relations séparées du plan image : ${topImagePlaneRelations}`} Les relations de guides ne sont ni des rapports harmoniques ni des mesures du monde réel.`,
        }],
        structuredContent,
        _meta: {
          normaPersonalVisualHarmony: {
...
const state={payload:null,proposalCandidateSetIdentity:null,proposalCandidates:[],reviewedCandidates:[],selected:new Set(),selectedGuides:new Set(),visibleKinds:new Set(["rectangle","segment","axis","ellipse"]),imageReady:false,dimensions:null,downloadUrl:null,completed:false,confirming:false};
...
function isStoredGeometrySnapshot(value,candidates){return Array.isArray(value)&&value.length===candidates.length&&value.every((item,index)=>{const candidate=candidates[index];return item&&candidate&&Object.keys(item).sort().join("|")==="height|id|width|x|y"&&item.id===candidate.id&&Number.isFinite(item.x)&&Number.isFinite(item.y)&&Number.isFinite(item.width)&&Number.isFinite(item.height)&&item.x>=0&&item.y>=0&&item.width>=0&&item.height>=0&&item.x+item.width<=1.000001&&item.y+item.height<=1.000001})}
...
function appendImagePlaneRelations(analysis){if(!analysis||!Array.isArray(analysis.relationships))return;for(const item of analysis.relationships.slice(0,3)){const label=item.classification==="intersection"?"COUPE":item.classification==="near_tangent"?"≈ TANGENTE":"PROCHE";appendMatchCard(label,item.ellipseLabel+" ↔ "+item.lineLabel,item.explanation)}if(isStoredIdentity(analysis.contentIdentity))identityNode.textContent+=" · plan-image "+analysis.contentIdentity}
...
const result=payload.result||structured,analysis=payload.imagePlaneGuideAnalysis||structured?.imagePlaneGuideAnalysis||null,confirmedGuideCount=analysis?.confirmedVisualGuideCandidateIds?.length||0,relationCount=analysis?.relationships?.length||0;
stageNode.textContent=revalidated?"MESURES REVALIDÉES":relationCount>0?"CORE + PLAN IMAGE VÉRIFIÉS":"CORE VÉRIFIÉ";stageNode.classList.add("done");
statusNode.textContent=(revalidated?"Sélection, corrections et dimensions revalidées par le serveur.":"Sélection et corrections reçues depuis ce widget · calcul déterministe terminé.")+(confirmedGuideCount>0?" "+confirmedGuideCount+" guide"+(confirmedGuideCount===1?"":"s")+" confirmé"+(confirmedGuideCount===1?"":"s")+" hors du Core rectangle; "+relationCount+" relation"+(relationCount===1?"":"s")+" du plan image mesurée"+(relationCount===1?"":"s")+".":"");
const explanations=(result.explanations||structured?.matches||[]).slice(0,5),canonicalResultIdentity=result.contentIdentity||structured?.canonicalResultIdentity||"",presentation=structured?.presentation||result.presentation||null;renderFacts(result.headline||structured?.headline,explanations,canonicalResultIdentity,"result.json",presentation);appendImagePlaneRelations(analysis);
