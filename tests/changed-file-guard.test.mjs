...
import {
  ...
  oauthImmediateRevocationLiveProofTruthSyncChangedFiles,
...
test("direct manual A/B phase two is an exact set with bounded CI repairs", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonyDirectSegmentPairChangedFiles),
    personalVisualHarmonyDirectSegmentPairChangedFiles,
  );
  ...
  for (const missingFile of [
    ...
    assert.equal(
      sharedExactApprovedChangedFiles(
        personalVisualHarmonyDirectSegmentPairChangedFiles.filter((file) => file !== missingFile),
      ),
      null,
      missingFile,
    );
...
test("completed result preview visibility is one exact widget-only change set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonyCompletedPreviewVisibilityChangedFiles),
    personalVisualHarmonyCompletedPreviewVisibilityChangedFiles,
  );
  assert.equal(
    sharedExactApprovedChangedFiles(personalVisualHarmonyCompletedPreviewVisibilityChangedFiles.slice(1)),
    null,
  );
  for (const forbiddenFile of [
...
test("malformed quadrilateral recovery is one exact bounded change set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonyMalformedQuadrilateralRecoveryChangedFiles),
    personalVisualHarmonyMalformedQuadrilateralRecoveryChangedFiles,
  );
  for (const missingFile of personalVisualHarmonyMalformedQuadrilateralRecoveryChangedFiles) {
    assert.equal(
      sharedExactApprovedChangedFiles(
        personalVisualHarmonyMalformedQuadrilateralRecoveryChangedFiles.filter((file) => file !== missingFile),
      ),
      null,
      missingFile,
    );
...
test("guide editing recovery is one exact bounded change set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonyGuideEditingRecoveryChangedFiles),
    personalVisualHarmonyGuideEditingRecoveryChangedFiles,
  );
  for (const forbiddenFile of [
    "src/personal-visual-harmony.ts",
...
test("repeat guide editing is one exact bounded change set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonyRepeatGuideEditingChangedFiles),
    personalVisualHarmonyRepeatGuideEditingChangedFiles,
  );
});
...
test("PR257 performance truth harness is an exact provider-free scoped set", () => {
  ...
  for (const forbiddenFile of [
    ...
    assert.equal(
      sharedExactApprovedChangedFiles([...performanceTruthHarnessChangedFiles, forbiddenFile]),
      null,
      forbiddenFile,
    );
  }
...
test("performance truth baseline runner is an exact provider-free scoped set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(performanceTruthBaselineRunnerChangedFiles),
    performanceTruthBaselineRunnerChangedFiles,
  );
  for (const forbiddenFile of [
    ...
    assert.equal(
      sharedExactApprovedChangedFiles([...performanceTruthBaselineRunnerChangedFiles, forbiddenFile]),
      null,
      forbiddenFile,
    );
  }
...
test("PR266 provider-neutral authorization data boundary is an exact sandbox implementation set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(pr266ProviderNeutralAuthorizationDataBoundaryChangedFiles),
    pr266ProviderNeutralAuthorizationDataBoundaryChangedFiles,
  );
  for (const forbiddenFile of [
    "package.json",
    ...
    "railway.json",
    "supabase/config.toml",
    "supabase/migrations/00000000000000_pr266.sql",
    "src/index.ts",
    "src/mcp/personal-visual-harmony-app.ts",
  ]) {
    assert.equal(
...
test("PostgreSQL authorization transaction race fix is an exact focused set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(postgreSqlAuthorizationTransactionRaceFixChangedFiles),
    postgreSqlAuthorizationTransactionRaceFixChangedFiles,
  );
  for (const forbiddenFile of [
    "src/mcp/remote-http-server.ts",
...
test("Railway to PostgreSQL sandbox vertical slice is an exact provider-neutral set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(railwayPostgreSqlSandboxVerticalSliceChangedFiles),
    railwayPostgreSqlSandboxVerticalSliceChangedFiles,
  );
  for (const forbiddenFile of [
    "package.json",
    ...
    assert.equal(
      sharedExactApprovedChangedFiles([...railwayPostgreSqlSandboxVerticalSliceChangedFiles, forbiddenFile]),
      null,
      forbiddenFile,
    );
  }
...
test("Sandbox qualification launch gates are an exact provider-free set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(sandboxQualificationLaunchGatesChangedFiles),
    sandboxQualificationLaunchGatesChangedFiles,
  );
  for (const forbiddenFile of [
    "package.json",
    ...
    assert.equal(
      sharedExactApprovedChangedFiles([...sandboxQualificationLaunchGatesChangedFiles, forbiddenFile]),
      null,
      forbiddenFile,
    );
  }
...
test("Stacked sandbox vertical slice plus qualification gates is an explicit aggregate set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles([
      ...railwayPostgreSqlSandboxVerticalSliceChangedFiles,
      ...sandboxQualificationLaunchGatesChangedFiles,
...
test("PostgreSQL runtime pool wiring is an exact Railway entrypoint set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(postgreSqlRuntimePoolWiringChangedFiles),
    postgreSqlRuntimePoolWiringChangedFiles,
  );
  for (const forbiddenFile of [
    "railway.json",
    ...
    "supabase/migrations/00000000000000_sandbox.sql",
    "src/index.ts",
    "docs/audits/2026-07-25-sandbox-qualification-session-handoff.md",
  ]) {
    assert.equal(
...
test("OAuth immediate revocation review hotfix has one exact security set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(oauthImmediateRevocationReviewHotfixChangedFiles),
    oauthImmediateRevocationReviewHotfixChangedFiles,
  );
  assert.equal(
    sharedExactApprovedChangedFiles([
...
test("OAuth immediate revocation merged-main proof sync has one exact docs set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(oauthImmediateRevocationLiveProofTruthSyncChangedFiles),
    oauthImmediateRevocationLiveProofTruthSyncChangedFiles,
  );
  assert.equal(
    sharedExactApprovedChangedFiles([
...
test("Scalekit scope alias qualification is an exact provider-neutral sandbox set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(scalekitScopeAliasQualificationChangedFiles),
    scalekitScopeAliasQualificationChangedFiles,
  );
  for (const forbiddenFile of [
    "package.json",
    ...
    "railway.json",
    "supabase/config.toml",
    "supabase/migrations/00000000000000_scalekit_scope_alias.sql",
  ]) {
    assert.equal(
      sharedExactApprovedChangedFiles([...scalekitScopeAliasQualificationChangedFiles, forbiddenFile]),
      null,
      forbiddenFile,
    );
  }
...
test("PR261 remote MCP OAuth tool security is an exact sandbox set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(pr261RemoteMcpOAuthToolSecurityChangedFiles),
    pr261RemoteMcpOAuthToolSecurityChangedFiles,
  );
  for (const forbiddenFile of [
    "src/mcp/remote-http-auth.ts",
    ...
    assert.equal(
      sharedExactApprovedChangedFiles([...pr261RemoteMcpOAuthToolSecurityChangedFiles, forbiddenFile]),
      null,
      forbiddenFile,
    );
  }
...
test("PR262 protected-resource root alias is an exact sandbox set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(pr262McpProtectedResourceRootAliasChangedFiles),
    pr262McpProtectedResourceRootAliasChangedFiles,
  );
  for (const forbiddenFile of [
    "src/mcp/remote-http-auth.ts",
    ...
    assert.equal(
      sharedExactApprovedChangedFiles([...pr262McpProtectedResourceRootAliasChangedFiles, forbiddenFile]),
      null,
      forbiddenFile,
    );
  }
...
test("PR263 Scalekit issuer compatibility is an exact auth sandbox set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(pr263ScalekitIssuerCompatibilityChangedFiles),
    pr263ScalekitIssuerCompatibilityChangedFiles,
  );
  for (const forbiddenFile of [
    "src/mcp/remote-http-server.ts",
...
test("PR265 remote visual MCP sandbox is an exact UI qualification set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(pr265RemoteVisualMcpSandboxChangedFiles),
    pr265RemoteVisualMcpSandboxChangedFiles,
  );
  for (const forbiddenFile of [
    "src/mcp/remote-http-auth.ts",
    ...
    "railway.json",
    "supabase/config.toml",
    "supabase/migrations/00000000000000_pr265.sql",
  ]) {
    assert.equal(
      sharedExactApprovedChangedFiles([...pr265RemoteVisualMcpSandboxChangedFiles, forbiddenFile]),
      null,
      forbiddenFile,
    );
  }
...
test("personal axis measurement-ratio allowlist is exact and fail-closed", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonyAxisMeasurementRatioChangedFiles),
    personalVisualHarmonyAxisMeasurementRatioChangedFiles,
  );
  for (const missingFile of personalVisualHarmonyAxisMeasurementRatioChangedFiles) {
    assert.equal(
      sharedExactApprovedChangedFiles(
        personalVisualHarmonyAxisMeasurementRatioChangedFiles.filter(
...
test("personal measurement-ratio selector clarity allowlist is exact and fail-closed", () => {
  ...
  for (const extra of [
    ...
    const approvedSet = sharedExactApprovedChangedFiles([
      ...personalVisualHarmonyMeasurementRatioClarityChangedFiles,
      extra,
    ]);
    if (extra === "tests/personal-visual-harmony-http.test.mjs") {
      assert.deepEqual(approvedSet, personalVisualHarmonyObservabilityChangedFiles);
    } else {
      assert.equal(approvedSet, null, extra);
    }
  }
});
...
test("personal observability and handler timing allowlist is exact and fail-closed", () => {
  ...
  for (const missingFile of personalVisualHarmonyObservabilityChangedFiles) {
    ...
    if (missingFile === "tests/personal-visual-harmony-http.test.mjs") {
      assert.deepEqual(approvedSubset, personalVisualHarmonyMeasurementRatioClarityChangedFiles);
    } else {
...
test("stale widget instance suppression retains the exact widget-only allowlist", () => {
  assert.equal(
    personalVisualHarmonyStaleWidgetInstanceChangedFiles,
    personalVisualHarmonyWidgetEllipseResponsiveChangedFiles,
  );
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonyStaleWidgetInstanceChangedFiles),
    personalVisualHarmonyStaleWidgetInstanceChangedFiles,
  );
});

test("personal widget runtime debugging retains one exact combined allowlist", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonyWidgetRuntimeDebugChangedFiles),
    personalVisualHarmonyWidgetRuntimeDebugChangedFiles,
  );
  for (const forbiddenFile of [
    "src/personal-visual-harmony-segmentation.ts",
...
test("personal hybrid perception allowlist is exact and keeps provider runtime outside PR1", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonyHybridPerceptionChangedFiles),
    personalVisualHarmonyHybridPerceptionChangedFiles,
  );
  assert.equal(
    sharedExactApprovedChangedFiles(personalVisualHarmonyHybridPerceptionChangedFiles.slice(1)),
    null,
  );
  for (const extra of [
    "src/index.ts",
...
test("SAM 3 semantic target toolbox allowlist is exact and rejects scope drift", () => {
  ...
  for (const missingFile of personalVisualHarmonySemanticTargetToolboxChangedFiles) {
    assert.equal(
      sharedExactApprovedChangedFiles(
        personalVisualHarmonySemanticTargetToolboxChangedFiles.filter(
          (file) => file !== missingFile,
        ),
      ),
      null,
...
test("measured review UX allowlist is exact and rejects scope drift", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonyMeasuredReviewUxChangedFiles),
    personalVisualHarmonyMeasuredReviewUxChangedFiles,
  );
  for (const missingFile of personalVisualHarmonyMeasuredReviewUxChangedFiles) {
    assert.equal(
      sharedExactApprovedChangedFiles(
        personalVisualHarmonyMeasuredReviewUxChangedFiles.filter(
          (file) => file !== missingFile,
        ),
      ),
      null,
...
test("declared spatial measurements allowlist is exact and rejects scope drift", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(declaredSpatialMeasurementsChangedFiles),
    declaredSpatialMeasurementsChangedFiles,
  );
  for (const missingFile of declaredSpatialMeasurementsChangedFiles) {
    assert.equal(
      sharedExactApprovedChangedFiles(
        declaredSpatialMeasurementsChangedFiles.filter((file) => file !== missingFile),
      ),
      null,
      missingFile,
    );
...
test("private Web Lab local CV candidates preserve the exact v1 provenance boundary", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(privateWebLabLocalCvCandidatesChangedFiles),
    privateWebLabLocalCvCandidatesChangedFiles,
  );
  for (const missingFile of privateWebLabLocalCvCandidatesChangedFiles) {
    assert.notDeepEqual(
      sharedExactApprovedChangedFiles(
        privateWebLabLocalCvCandidatesChangedFiles.filter((file) => file !== missingFile),
      ),
      privateWebLabLocalCvCandidatesChangedFiles,
      missingFile,
    );
...
test("personal visual harmony perception timeout recovery allowlist is exact and rejects scope drift", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonyPerceptionTimeoutRecoveryChangedFiles),
    personalVisualHarmonyPerceptionTimeoutRecoveryChangedFiles,
  );
  for (const missingFile of personalVisualHarmonyPerceptionTimeoutRecoveryChangedFiles) {
    assert.equal(
      sharedExactApprovedChangedFiles(
        personalVisualHarmonyPerceptionTimeoutRecoveryChangedFiles.filter(
          (file) => file !== missingFile,
        ),
      ),
      null,
...
test("personal visual harmony SAM cold-start async allowlist is exact and rejects scope drift", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonySam3ColdStartAsyncChangedFiles),
    personalVisualHarmonySam3ColdStartAsyncChangedFiles,
  );
...
test("personal visual harmony generic V1 SAM capability allowlist is exact and rejects scope drift", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonySamGenericCapabilityChangedFiles),
    personalVisualHarmonySamGenericCapabilityChangedFiles,
  );
...
test("private Web Lab allowlist is exact and rejects scope drift", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(privateWebLabChangedFiles),
    privateWebLabChangedFiles,
  );
  for (const missingFile of privateWebLabChangedFiles) {
    assert.equal(
      sharedExactApprovedChangedFiles(
        privateWebLabChangedFiles.filter((file) => file !== missingFile),
      ),
      null,
    );
  }
...
test("private Web Lab manual authoring allowlist is exact and rejects scope drift", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(privateWebLabManualAuthoringChangedFiles),
    privateWebLabManualAuthoringChangedFiles,
  );
  for (const missingFile of privateWebLabManualAuthoringChangedFiles) {
    assert.equal(
      sharedExactApprovedChangedFiles(
        privateWebLabManualAuthoringChangedFiles.filter((file) => file !== missingFile),
      ),
      null,
    );
  }
...
test("private Web Lab authoring status test fix preserves its exact test-only boundary", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(privateWebLabAuthoringStatusTestFixChangedFiles),
    privateWebLabAuthoringStatusTestFixChangedFiles,
  );
  for (const missingFile of privateWebLabAuthoringStatusTestFixChangedFiles) {
    assert.equal(
      sharedExactApprovedChangedFiles(
        privateWebLabAuthoringStatusTestFixChangedFiles.filter((file) => file !== missingFile),
      ),
      null,
      missingFile,
    );
...
test("private Web Lab manual precision allowlist is exact and rejects scope drift", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(privateWebLabManualPrecisionChangedFiles),
    privateWebLabManualPrecisionChangedFiles,
  );
  for (const missingFile of privateWebLabManualPrecisionChangedFiles) {
    assert.equal(
      sharedExactApprovedChangedFiles(
        privateWebLabManualPrecisionChangedFiles.filter((file) => file !== missingFile),
      ),
      null,
    );
  }
...
test("private Web Lab result presentation allowlist is exact and rejects scope drift", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(privateWebLabResultPresentationChangedFiles),
    privateWebLabResultPresentationChangedFiles,
  );
  for (const missingFile of privateWebLabResultPresentationChangedFiles) {
    if (missingFile === "web-lab/private-web-lab-browser-model.js") {
      assert.deepEqual(
        sharedExactApprovedChangedFiles(
          privateWebLabResultPresentationChangedFiles.filter((file) => file !== missingFile),
        ),
        privateWebLabManualPrecisionChangedFiles,
      );
      continue;
...
test("private Web Lab session recovery launcher allowlist is exact and rejects scope drift", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(privateWebLabSessionRecoveryLauncherChangedFiles),
    privateWebLabSessionRecoveryLauncherChangedFiles,
  );
  for (const missingFile of privateWebLabSessionRecoveryLauncherChangedFiles) {
    assert.equal(
      sharedExactApprovedChangedFiles(
        privateWebLabSessionRecoveryLauncherChangedFiles.filter(
          (file) => file !== missingFile,
        ),
      ),
      null,
...
test("SAM 3 Modal checkpoint packaging allowlist is exact and rejects scope drift", () => {
  assert.deepEqual(personalVisualHarmonySam3ModalPackagingFixChangedFiles, [
    "deploy/modal/modal_app.py",
  ...
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonySam3ModalPackagingFixChangedFiles),
    personalVisualHarmonySam3ModalPackagingFixChangedFiles,
  );
  assert.equal(
    sharedExactApprovedChangedFiles(
      personalVisualHarmonySam3ModalPackagingFixChangedFiles.slice(1),
    ),
    null,
  );
  for (const extra of [
...
test("stacked Railway admission, Modal readiness, and checkpoint packaging is exact", () => {
  assert.deepEqual(
    statelessMcpScaffoldingQuotaFixSam3ModalReadinessCheckpointPackagingChangedFiles,
    [
      "deploy/modal/modal_app.py",
...
test("MCP App resource metadata, app-only decoupling, cache-bust, legacy MIME, and stale-read allowlist is exact and rejects scope drift", () => {
  assert.deepEqual(
  ...
  assert.equal(
    sharedExactApprovedChangedFiles(
      personalVisualHarmonyMcpAppResourceMetadataChangedFiles.slice(1),
    ),
    null,
  );
  for (const extra of [
...
test("widget publication metadata is an exact bounded change set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(personalVisualHarmonyWidgetPublicationMetadataChangedFiles),
    personalVisualHarmonyWidgetPublicationMetadataChangedFiles,
  );
  assert.equal(
    sharedExactApprovedChangedFiles(
      personalVisualHarmonyWidgetPublicationMetadataChangedFiles.slice(1),
    ),
    null,
  );
  for (const extra of [
...
test("post-PR318 private observation truth sync is an exact docs/tests change set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(postPr318PrivateObservationTruthSyncChangedFiles),
    postPr318PrivateObservationTruthSyncChangedFiles,
  );
  assert.equal(
    sharedExactApprovedChangedFiles(postPr318PrivateObservationTruthSyncChangedFiles.slice(1)),
    null,
  );
  for (const extra of [
    "src/mcp/personal-visual-harmony-app.ts",
...
test("post-PR319 live browser observation is an exact docs/tests change set", () => {
  assert.deepEqual(
    sharedExactApprovedChangedFiles(postPr319LiveBrowserObservationChangedFiles),
    postPr319LiveBrowserObservationChangedFiles,
  );
  assert.equal(
    sharedExactApprovedChangedFiles(postPr319LiveBrowserObservationChangedFiles.slice(1)),
    null,
  );
  for (const extra of [
    "src/mcp/personal-visual-harmony-app.ts",
...
test("shared exact changed-file guard recognizes active controlled provider observation proof branches", () => {
  ...
  const isDeclaredSpatialMeasurementsSet = isExactChangedFileSet(
    changedFiles,
    declaredSpatialMeasurementsChangedFiles,
  );
  const isPersonalVisualHarmonyObservabilitySet = isExactChangedFileSet(
    changedFiles,
    personalVisualHarmonyObservabilityChangedFiles,
  );
  ...
  const isPersonalVisualHarmonyWidgetRuntimeDebugSet = isExactChangedFileSet(
    changedFiles,
    personalVisualHarmonyWidgetRuntimeDebugChangedFiles,
  );
  ...
  const isPersonalVisualHarmonyMeasuredReviewUxSet = isExactChangedFileSet(
    changedFiles,
    personalVisualHarmonyMeasuredReviewUxChangedFiles,
  );
  const isPrivateWebLabSet = isExactChangedFileSet(
    changedFiles,
    privateWebLabChangedFiles,
  );
  const isPrivateWebLabLocalCvCandidatesSet = isExactChangedFileSet(
    changedFiles,
    privateWebLabLocalCvCandidatesChangedFiles,
  );
  const isPersonalVisualHarmonyTwoObjectSpatialSet =
    ...
    isExactChangedFileSet(changedFiles, personalVisualHarmonySamGenericCapabilityChangedFiles);
  const isPersonalVisualHarmonyPerceptionTimeoutRecoverySet = isExactChangedFileSet(
    changedFiles,
    personalVisualHarmonyPerceptionTimeoutRecoveryChangedFiles,
  );
  const isPersonalVisualHarmonySam3ColdStartAsyncSet = isExactChangedFileSet(
    changedFiles,
    personalVisualHarmonySam3ColdStartAsyncChangedFiles,
  );
  ...
  const isPrivateWebLabAuthoringStatusTestFixSet = isExactChangedFileSet(
    changedFiles,
    privateWebLabAuthoringStatusTestFixChangedFiles,
  );
  const isPrivateWebLabManualPrecisionSet = isExactChangedFileSet(
    changedFiles,
    privateWebLabManualPrecisionChangedFiles,
  );
  const isPrivateWebLabResultPresentationSet = isExactChangedFileSet(
    changedFiles,
    privateWebLabResultPresentationChangedFiles,
  );
  const isPrivateWebLabSessionRecoveryLauncherSet = isExactChangedFileSet(
    changedFiles,
    privateWebLabSessionRecoveryLauncherChangedFiles,
  );
  const isPersonalVisualHarmonySam3ModalPackagingFixSet = isExactChangedFileSet(
    changedFiles,
    personalVisualHarmonySam3ModalPackagingFixChangedFiles,
  );
  const isStatelessMcpScaffoldingQuotaFixSam3ModalReadinessCheckpointPackagingSet =
    isExactChangedFileSet(
      changedFiles,
      statelessMcpScaffoldingQuotaFixSam3ModalReadinessCheckpointPackagingChangedFiles,
    );
  ...
  const isStatelessMcpScaffoldingQuotaFixSam3TerminalDiagnosticSet = isExactChangedFileSet(
    changedFiles,
    statelessMcpScaffoldingQuotaFixSam3TerminalDiagnosticChangedFiles,
  );
  ...
  const isPersonalVisualHarmonyWidgetPublicationMetadataSet = isExactChangedFileSet(
    changedFiles,
    personalVisualHarmonyWidgetPublicationMetadataChangedFiles,
  );
  const isPostPr318PrivateObservationTruthSyncSet = isExactChangedFileSet(
    changedFiles,
    postPr318PrivateObservationTruthSyncChangedFiles,
  );
  const isPostPr319LiveBrowserObservationSet = isExactChangedFileSet(
    changedFiles,
    postPr319LiveBrowserObservationChangedFiles,
  );
  ...
  const isPersonalVisualHarmonyDirectSegmentPairSet = isExactChangedFileSet(
    changedFiles,
    personalVisualHarmonyDirectSegmentPairChangedFiles,
  );
  ...
  const isPersonalVisualHarmonyCompletedPreviewVisibilitySet = isExactChangedFileSet(
    changedFiles,
    personalVisualHarmonyCompletedPreviewVisibilityChangedFiles,
  );
  const isPersonalVisualHarmonyMalformedQuadrilateralRecoverySet = isExactChangedFileSet(
    changedFiles,
    personalVisualHarmonyMalformedQuadrilateralRecoveryChangedFiles,
  );
  const isPersonalVisualHarmonyGuideEditingRecoverySet = isExactChangedFileSet(
    changedFiles,
    personalVisualHarmonyGuideEditingRecoveryChangedFiles,
  );
  const isPersonalVisualHarmonyRepeatGuideEditingSet = isExactChangedFileSet(
    changedFiles,
    personalVisualHarmonyRepeatGuideEditingChangedFiles,
  );
  ...
  const isPerformanceTruthBaselineRunnerSet = isExactChangedFileSet(
    changedFiles,
    performanceTruthBaselineRunnerChangedFiles,
  );
  ...
  const isPr261Set = isExactChangedFileSet(
    changedFiles,
    pr261RemoteMcpOAuthToolSecurityChangedFiles,
  );
  const isPr262Set = isExactChangedFileSet(
    changedFiles,
    pr262McpProtectedResourceRootAliasChangedFiles,
  );
  const isScalekitScopeAliasSet = isExactChangedFileSet(
    changedFiles,
    scalekitScopeAliasQualificationChangedFiles,
  );
  const isPr263Set = isExactChangedFileSet(
    changedFiles,
    pr263ScalekitIssuerCompatibilityChangedFiles,
  );
  const isPr264Set = isExactChangedFileSet(
    changedFiles,
    pr264ScalekitCompositeAudienceChangedFiles,
  );
  const isPr265Set = isExactChangedFileSet(
    changedFiles,
    pr265RemoteVisualMcpSandboxChangedFiles,
  );
  const isPr266Set = isExactChangedFileSet(
    changedFiles,
    pr266ProviderNeutralAuthorizationDataBoundaryChangedFiles,
  );
  ...
  const isPostgreSqlAuthorizationTransactionRaceFixSet = isExactChangedFileSet(
    changedFiles,
    postgreSqlAuthorizationTransactionRaceFixChangedFiles,
  );
  const isRailwayPostgreSqlSandboxVerticalSliceSet = isExactChangedFileSet(
    changedFiles,
    railwayPostgreSqlSandboxVerticalSliceChangedFiles,
  );
  const isSandboxQualificationLaunchGatesSet = isExactChangedFileSet(
    changedFiles,
    sandboxQualificationLaunchGatesChangedFiles,
  );
  const isRailwayPostgreSqlSandboxAndQualificationSet = isExactChangedFileSet(
    changedFiles,
    railwayPostgreSqlSandboxAndQualificationChangedFiles,
  );
  const isPostgreSqlRuntimePoolWiringSet = isExactChangedFileSet(
    changedFiles,
    postgreSqlRuntimePoolWiringChangedFiles,
  );
  const isPostgreSqlSandboxTlsAndPoolResetFixSet = isExactChangedFileSet(
    changedFiles,
    postgreSqlSandboxTlsAndPoolResetFixChangedFiles,
  );
  ...
  const isOAuthImmediateRevocationReviewHotfixSet = isExactChangedFileSet(
    changedFiles,
    oauthImmediateRevocationReviewHotfixChangedFiles,
  );
  const isOAuthImmediateRevocationLiveProofTruthSyncSet = isExactChangedFileSet(
    changedFiles,
    oauthImmediateRevocationLiveProofTruthSyncChangedFiles,
  );
  ...
  if (isPersonalVisualHarmonyCompletedPreviewVisibilitySet) {
    assert.deepEqual(
      sharedExactApprovedChangedFiles(changedFiles),
      personalVisualHarmonyCompletedPreviewVisibilityChangedFiles,
    );
  ...
  if (isPersonalVisualHarmonyMalformedQuadrilateralRecoverySet) {
    assert.deepEqual(
      sharedExactApprovedChangedFiles(changedFiles),
      personalVisualHarmonyMalformedQuadrilateralRecoveryChangedFiles,
    );
  ...
  if (isPersonalVisualHarmonyGuideEditingRecoverySet) {
    assert.deepEqual(
      sharedExactApprovedChangedFiles(changedFiles),
      personalVisualHarmonyGuideEditingRecoveryChangedFiles,
    );
  ...
  if (isPersonalVisualHarmonyRepeatGuideEditingSet) {
    assert.deepEqual(
      sharedExactApprovedChangedFiles(changedFiles),
      personalVisualHarmonyRepeatGuideEditingChangedFiles,
    );
  ...
  assert.deepEqual(
    ...
    isDeclaredImagePlaneMeasurementRatiosSet
      ...
      ? personalVisualHarmonyPerceptionTimeoutRecoveryChangedFiles
      : isPersonalVisualHarmonySam3ColdStartAsyncSet
      ? personalVisualHarmonySam3ColdStartAsyncChangedFiles
      ...
      ? privateWebLabManualPrecisionChangedFiles
      : isPrivateWebLabResultPresentationSet
      ? privateWebLabResultPresentationChangedFiles
      : isPrivateWebLabSessionRecoveryLauncherSet
      ? privateWebLabSessionRecoveryLauncherChangedFiles
      : isPersonalVisualHarmonySam3ModalPackagingFixSet
      ? personalVisualHarmonySam3ModalPackagingFixChangedFiles
      ...
      ? statelessMcpScaffoldingQuotaFixSam3ModalReadinessCheckpointPackagingMultiInstanceChangedFiles
      : isStatelessMcpScaffoldingQuotaFixSam3TerminalDiagnosticSet
      ? statelessMcpScaffoldingQuotaFixSam3TerminalDiagnosticChangedFiles
      : isPersonalVisualHarmonyMcpAppResourceMetadataSet
      ? personalVisualHarmonyMcpAppResourceMetadataChangedFiles
      : isPersonalVisualHarmonyWidgetPublicationMetadataSet
      ? personalVisualHarmonyWidgetPublicationMetadataChangedFiles
      : isPostPr318PrivateObservationTruthSyncSet
      ? postPr318PrivateObservationTruthSyncChangedFiles
      : isPostPr319LiveBrowserObservationSet
      ? postPr319LiveBrowserObservationChangedFiles
      ...
      ? personalVisualHarmonyManualSegmentChangedFiles
      : isPersonalVisualHarmonyDirectSegmentPairSet
      ? personalVisualHarmonyDirectSegmentPairChangedFiles
      : isPersonalVisualHarmonyMalformedQuadrilateralRecoverySet
      ? personalVisualHarmonyMalformedQuadrilateralRecoveryChangedFiles
      : isPersonalVisualHarmonyGuideEditingRecoverySet
      ? personalVisualHarmonyGuideEditingRecoveryChangedFiles
      : isPersonalVisualHarmonyRepeatGuideEditingSet
      ...
      : isPr257Set
      ? performanceTruthHarnessChangedFiles
      : isPerformanceTruthBaselineRunnerSet
      ? performanceTruthBaselineRunnerChangedFiles
      : isPr258Set
      ? pr258RlsBoundaryMcpSandboxQualificationChangedFiles
      : isPr259Set
      ...
      : isPr266Set
      ? pr266ProviderNeutralAuthorizationDataBoundaryChangedFiles
      : isPostgreSqlAuthorizationTransactionRaceFixSet
      ? postgreSqlAuthorizationTransactionRaceFixChangedFiles
      : isPostgreSqlAuthorizationTransactionSet
      ? postgreSqlAuthorizationTransactionBoundaryChangedFiles
      : isRailwayPostgreSqlSandboxAndQualificationSet
      ? railwayPostgreSqlSandboxAndQualificationChangedFiles
      : isPostgreSqlRuntimePoolWiringSet
      ? postgreSqlRuntimePoolWiringChangedFiles
      : isPostgreSqlSandboxTlsAndPoolResetFixSet
      ? postgreSqlSandboxTlsAndPoolResetFixChangedFiles
      : isOAuthImmediateRevocationSet
      ? oauthImmediateRevocationChangedFiles
      : isOAuthImmediateRevocationReviewHotfixSet
      ? oauthImmediateRevocationReviewHotfixChangedFiles
      : isOAuthImmediateRevocationLiveProofTruthSyncSet
      ? oauthImmediateRevocationLiveProofTruthSyncChangedFiles
